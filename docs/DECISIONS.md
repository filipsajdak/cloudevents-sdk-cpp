...
GCC 16 **rejects `-std=c++26`**; its spelling is `-std=c++2c`. `-freflection` is in
turn rejected at `-std=c++20` and `-std=c++23` (`'-freflection' only supported with
...
## D-CONFIG-2: C++26 is selected by explicit flag, not by `CMAKE_CXX_STANDARD`

`CMAKE_CXX_STANDARD 26` emits `-std=c++26`, which GCC 16 rejects. The reflection
preset therefore sets `-std=c++2c -freflection` through `target_compile_options`
rather than through the standard property. `ce::core` declares only
`target_compile_features(... cxx_std_20)` as its floor.
