...
void describe(const ce::event& subject) {
  ...
  if (auto tracing = subject.get<ce::ext::tracing>()) {
...
[[nodiscard]] auto receive(const ce::message& request) -> int {
  switch (ce::http::detect_content_mode(request)) {
    case ce::content_mode::batched: {
      auto events = ce::http::from_batch_message<codec>(request);
      if (!events) {
    ...
    case ce::content_mode::structured:
    case ce::content_mode::binary_mode: {
      auto subject = ce::http::from_message<codec>(request);
      if (!subject) {
        ...
        if (subject.error().code == ce::errc::not_a_cloudevent) {
...
[[nodiscard]] auto binary_request() -> ce::message {
  ce::message request;
  ...
  request.body = ce::http::detail::to_bytes(R"({"total":42})");
  return request;
}

[[nodiscard]] auto structured_request() -> ce::message {
  ce::message request;
  request.header_fields.add("Content-Type", "application/cloudevents+json");
  request.body = ce::http::detail::to_bytes(
      R"({"specversion":"1.0","id":"B1","source":"/s","type":"com.example.other"})");
  return request;
}

[[nodiscard]] auto not_an_event() -> ce::message {
  ce::message request;
  request.header_fields.add("Content-Type", "application/json");
  request.body = ce::http::detail::to_bytes(R"({"hello":"world"})");
  return request;
}
