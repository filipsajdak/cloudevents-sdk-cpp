...
int main() {
  ...
  static_assert(ce::v1::described<shop::order>);
  static_assert(ce::v1::members_supported<shop::order>());
  static_assert(ce::v1::field_count<shop::order> == 6);
  ...
  if (auto stored = ce::v1::set_data<shop::order, codec>(subject, placed); !stored) {
    std::fprintf(stderr, "set_data: %s\n", stored.error().detail.c_str());
    return 1;
  }

  auto encoded = ce::v1::json_format<codec>::encode(subject);
  if (!encoded) {
  ...
  auto received = ce::v1::json_format<codec>::decode(*encoded);
  if (!received) {
    std::fprintf(stderr, "decode: %s\n", received.error().detail.c_str());
    return 1;
  }

  auto payload = ce::v1::data_as<shop::order, codec>(*received);
  if (!payload) {
  ...
  auto view = ce::v1::event_of<shop::line_item, codec>::with_data(
  ...
  auto as_item = ce::v1::data_as<shop::line_item, codec>(*received);
  if (!as_item) {
  ...
  ce::v1::event mistyped{
      .id = "B1",
      .source = ce::v1::uri_ref{"/s"},
      .type = "com.example.order.line",
  };
  mistyped.datacontenttype = "application/json";
  mistyped.data = ce::v1::json_text{.raw = R"({"sku":"SKU-1","quantity":"two"})"};

  auto rejected = ce::v1::data_as<shop::line_item, codec>(mistyped);
  if (rejected) {
