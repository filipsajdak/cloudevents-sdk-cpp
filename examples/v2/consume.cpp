...
void describe(const ce::v2::event& subject) {
  ...
  std::visit(
      [](const auto& payload) {
        ...
        } else if constexpr (std::is_same_v<T, ce::v2::binary>) {
  ...
  if (auto tracing = subject.get<ce::v2::ext::tracing>()) {
...
[[nodiscard]] auto receive(const ce::v2::message& request) -> int {
  switch (ce::v2::http::detect_content_mode(request)) {
    case ce::v2::content_mode::batched: {
      auto events = ce::v2::http::from_batch_message<codec>(request);
      if (!events) {
    ...
    case ce::v2::content_mode::structured:
    case ce::v2::content_mode::binary_mode: {
      auto subject = ce::v2::http::from_message<codec>(request);
      if (!subject) {
        ...
        if (subject.error().code == ce::v2::errc::not_a_cloudevent) {
...
[[nodiscard]] auto binary_request() -> ce::v2::message {
  ...
  request.body = ce::v2::http::detail::to_bytes(R"({"total":42})");
  return request;
}

[[nodiscard]] auto structured_request() -> ce::v2::message {
  ce::v2::message request;
  request.header_fields.add("Content-Type", "application/cloudevents+json");
  request.body = ce::v2::http::detail::to_bytes(
      R"({"specversion":"1.0","id":"B1","source":"/s","type":"com.example.other"})");
  return request;
}

[[nodiscard]] auto not_an_event() -> ce::v2::message {
  ce::v2::message request;
  request.header_fields.add("Content-Type", "application/json");
  request.body = ce::v2::http::detail::to_bytes(R"({"hello":"world"})");
  return request;
}
