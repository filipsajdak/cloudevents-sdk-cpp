...
int main() {
  ...
  static_assert(ce::v2::described<shop::order>);
  static_assert(ce::v2::members_supported<shop::order>());
  static_assert(ce::v2::field_count<shop::order> == 6);
  ...
  ce::v2::event subject{"A234-1234-1234"_id, "https://example.test/orders"_source,
  ...
  ce::v2::set_data<shop::order, codec>(subject, placed);

  auto encoded = ce::v2::json_format<codec>::encode(subject);
  if (!encoded) {
  ...
  auto received = ce::v2::json_format<codec>::decode(*encoded);
  if (!received) {
    std::fprintf(stderr, "decode: %s\n", received.error().detail.c_str());
    return 1;
  }

  auto payload = ce::v2::data_as<shop::order, codec>(*received);
  if (!payload) {
  ...
  auto view = ce::v2::event_of<shop::line_item, codec>::with_data(
  ...
  auto as_item = ce::v2::data_as<shop::line_item, codec>(*received);
  if (!as_item) {
  ...
  const ce::v2::event mistyped{
      "B1"_id,
      "/s"_source,
      "com.example.order.line"_type,
      {
          .datacontenttype = "application/json"_mediatype,
          .data = ce::v2::json_text{.raw = R"({"sku":"SKU-1","quantity":"two"})"},
      },
  };

  auto rejected = ce::v2::data_as<shop::line_item, codec>(mistyped);
  if (rejected) {
