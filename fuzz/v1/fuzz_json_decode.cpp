...
#include "test/v1/mini_codec.hpp"

extern "C" auto LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) -> int {
  const std::string_view text{reinterpret_cast<const char*>(data), size};

  auto decoded = ce::v1::json_format<ce::v1::codec::nlohmann_codec>::decode(text);
  if (decoded) {
    ...
    if (!ce::v1::json_format<ce::v1::codec::nlohmann_codec>::encode(*decoded)) {
      __builtin_trap();
    }
  }
  ...
  (void)ce::v1::json_format<ce::v1::test::mini_codec>::decode(text);
  (void)ce::v1::json_format<ce::v1::codec::nlohmann_codec>::decode_batch(text);
  return 0;
}
