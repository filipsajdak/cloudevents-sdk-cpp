...
extern "C" auto LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) -> int {
  ...
  const ce::v1::message incoming = build(payload);
  (void)ce::v1::nats::detect_content_mode(incoming);
  if (auto from_headers = ce::v1::nats::from_message<ce::v1::codec::nlohmann_codec>(incoming)) {
    ...
    if (!ce::v1::nats::to_message<ce::v1::codec::nlohmann_codec>(*from_headers,
                                                         ce::v1::content_mode::binary_mode)) {
      __builtin_trap();
    }
  }

  auto decoded = ce::v1::nats::from_payload<ce::v1::codec::nlohmann_codec>(payload);
  if (!decoded) {
    return 0;
  }

  auto reserialized = ce::v1::nats::to_payload<ce::v1::codec::nlohmann_codec>(*decoded);
  if (!reserialized) {
    __builtin_trap();
  }

  auto again = ce::v1::nats::from_payload<ce::v1::codec::nlohmann_codec>(*reserialized);
