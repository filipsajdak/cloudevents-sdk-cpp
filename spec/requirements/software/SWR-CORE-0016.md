...
rationale: >
  ...
  invalid or reserved name, so storing one can no longer fail.
...
`attribute_value` and storing it without a failure path, alongside `extension`,
returning the stored value or an empty result when the name is absent.
