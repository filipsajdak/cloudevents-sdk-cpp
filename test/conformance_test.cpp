...
[[nodiscard]] auto parse_header_block(std::string_view text) -> ce::message {
  ce::message request;
  std::size_t pos = 0;
  bool in_body = false;
  std::string body;
...
void check_elided_base64(std::string_view label) {
  ...
  if (!single) {
    expect(single.error().code == ce::errc::invalid_base64) << label << ": json-01";
  }

  auto batch = format::decode_batch(read_fixture("json-09-batch.json"));
  expect(!batch.has_value()) << label << ": json-09 should not decode";
  if (!batch) {
    expect(batch.error().code == ce::errc::invalid_base64) << label << ": json-09";
  }
...
void check_literal_binary_headers(std::string_view label) {
  ...
  for (const auto name : {"json-04-binary-headers.http"sv, "json-06-binary-headers.http"sv,
    ...
    const ce::message request = parse_header_block(read_fixture(name));
    ...
    expect(ce::http::detect_content_mode(request) == ce::content_mode::binary_mode)
        << label << ": " << name;
...
void check_media_type_examples() {
  ...
  const ce::message structured = parse_header_block(read_fixture("http-03-structured-content-type.http"));
  expect(ce::http::detect_content_mode(structured) == ce::content_mode::structured);

  const ce::message batched = parse_header_block(read_fixture("http-06-batch-content-type.http"));
  expect(ce::http::detect_content_mode(batched) == ce::content_mode::batched);
...
void check_elided_exchanges() {
  ...
  const ce::message binary_request = parse_header_block(read_fixture("http-01-binary-request.http"));
  ...
  expect(ce::http::detect_content_mode(binary_request) == ce::content_mode::binary_mode);
  ...
  const ce::message batch_request = parse_header_block(read_fixture("http-07-batch-request.http"));
  expect(ce::http::detect_content_mode(batch_request) == ce::content_mode::batched);
