...
template <ce::described T>
void check(const expectation& expected) {
  using namespace boost::ut;

  expect(ce::field_count<T> == expected.wire_names.size())
      << expected.label << ": field_count";

  const auto names = ce::field_names<T>();
  ...
  ce::for_each_field(object, [&visited](std::string_view visited_name, auto&) {
  ...
  ce::for_each_field(const_object, [&const_visited](std::string_view visited_name, const auto&) {
