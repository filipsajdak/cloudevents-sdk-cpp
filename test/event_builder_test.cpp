...
[[nodiscard]] auto absent(const ce::result<ce::event>& built, std::string_view where) -> bool {
  return !built.has_value() && built.error().code == ce::errc::missing_required_attribute &&
...
const boost::ut::suite<"event-builder-requires-id-source-and-type"> event_builder = [] {
  ...
  "an empty builder reports id first"_test = [] {
    expect(absent(ce::event::builder{}.build(), "id"));
  };
  ...
  "the report names the first attribute that is absent"_test = [] {
    expect(absent(ce::event::builder{.source = "/s"_source, .type = "t"_type}.build(), "id"));
    expect(absent(ce::event::builder{.id = "1"_id, .type = "t"_type}.build(), "source"));
    expect(absent(ce::event::builder{.id = "1"_id, .source = "/s"_source}.build(), "type"));
  };

  "with all three it builds the event they describe"_test = [] {
    auto built = ce::event::builder{
        .id = "1"_id,
        .source = "/s"_source,
        .type = "t"_type,
    ...
    expect(*built == ce::event{"1"_id, "/s"_source, "t"_type, {.subject = "x"_subject}});
  };

  // Rvalue-qualified, so a builder cannot be built twice or left half-consumed.
  "build consumes the builder"_test = [] {
    static_assert(std::is_invocable_v<decltype(&ce::event::builder::build), ce::event::builder&&>);
    static_assert(!std::is_invocable_v<decltype(&ce::event::builder::build), ce::event::builder&>);
    expect(true);
  };
};
