...
// and ce::uri_ref, ce::event and the extension structs have no operator<<.
...
const boost::ut::suite<"extensions-described-structs"> described_structs = [] {
  ...
  "all five extensions are described"_test = [] {
    static_assert(ce::described<ce::ext::tracing>);
    static_assert(ce::described<ce::ext::partitioning>);
    static_assert(ce::described<ce::ext::sampled_rate>);
    static_assert(ce::described<ce::ext::sequence>);
    static_assert(ce::described<ce::ext::dataref>);
    expect(true);
  };
  ...
  "the macro backend describes them"_test = [] {
    ...
    static_assert(ce::backend_of<ce::ext::tracing> == ce::describe_backend::macro);
    static_assert(ce::backend_of<ce::ext::dataref> == ce::describe_backend::macro);
    expect(true);
  };

  "the wire names are the ones the CloudEvents spec defines"_test = [] {
    constexpr auto tracing = ce::field_names<ce::ext::tracing>();
    static_assert(ce::field_count<ce::ext::tracing> == 2);
    expect(tracing[0] == "traceparent"sv);
    expect(tracing[1] == "tracestate"sv);

    constexpr auto partitioning = ce::field_names<ce::ext::partitioning>();
    static_assert(ce::field_count<ce::ext::partitioning> == 1);
    expect(partitioning[0] == "partitionkey"sv);

    constexpr auto rate = ce::field_names<ce::ext::sampled_rate>();
    static_assert(ce::field_count<ce::ext::sampled_rate> == 1);
    ...
    constexpr auto sequence = ce::field_names<ce::ext::sequence>();
    static_assert(ce::field_count<ce::ext::sequence> == 1);
    expect(sequence[0] == "sequence"sv);

    constexpr auto dataref = ce::field_names<ce::ext::dataref>();
    static_assert(ce::field_count<ce::ext::dataref> == 1);
  ...
  "every wire name is a legal extension attribute name"_test = [] {
    ...
    for (const auto names :
         {ce::field_names<ce::ext::tracing>()[0], ce::field_names<ce::ext::tracing>()[1]}) {
      expect(ce::valid_attribute_name(names)) << names;
      expect(!ce::reserved_name(names)) << names;
    }
    expect(ce::valid_attribute_name(ce::field_names<ce::ext::partitioning>()[0]));
    expect(ce::valid_attribute_name(ce::field_names<ce::ext::sampled_rate>()[0]));
    expect(ce::valid_attribute_name(ce::field_names<ce::ext::sequence>()[0]));
    expect(ce::valid_attribute_name(ce::field_names<ce::ext::dataref>()[0]));
  };

  "the declared types are the CloudEvents attribute types"_test = [] {
    static_assert(std::is_same_v<decltype(ce::ext::tracing::traceparent), std::string>);
    static_assert(
        std::is_same_v<decltype(ce::ext::tracing::tracestate), std::optional<std::string>>);
    static_assert(std::is_same_v<decltype(ce::ext::partitioning::partitionkey), std::string>);
    // Integer, not a string: the spec says sampledrate is an Integer, and the
    // declared type is what get<> restores it to.
    static_assert(std::is_same_v<decltype(ce::ext::sampled_rate::sampledrate), std::int32_t>);
    static_assert(std::is_same_v<decltype(ce::ext::sequence::value), std::string>);
    // URI-reference, not a string, for the same reason.
    static_assert(std::is_same_v<decltype(ce::ext::dataref::value), ce::uri_ref>);
    expect(true);
  };

  "sampledrate carries the one constraint the type system cannot"_test = [] {
    expect(ce::ext::sampled_rate{.sampledrate = 1}.validate().has_value());
    expect(ce::ext::sampled_rate{.sampledrate = 30}.validate().has_value());
    for (const std::int32_t bad : {0, -1, -30}) {
      auto checked = ce::ext::sampled_rate{.sampledrate = bad}.validate();
      expect(!checked.has_value()) << bad;
      if (!checked) {
        expect(checked.error().code == ce::errc::invalid_attribute_value) << bad;
...
const boost::ut::suite<"extensions-get-set-roundtrip"> get_set_roundtrip = [] {
  ...
  "each extension reads back what it wrote"_test = [] {
    ...
    const ce::ext::tracing tracing{.traceparent = "00-0af7-00f0-01", .tracestate = "vendor=1"};
    expect(subject.set(tracing).has_value());
    auto read_tracing = subject.get<ce::ext::tracing>();
    expect(read_tracing.has_value());
    if (read_tracing) {
      expect(bool{*read_tracing == tracing});
    }

    const ce::ext::partitioning partitioning{.partitionkey = "customer-42"};
    expect(subject.set(partitioning).has_value());
    auto read_partitioning = subject.get<ce::ext::partitioning>();
    expect(read_partitioning.has_value());
    if (read_partitioning) {
      expect(bool{*read_partitioning == partitioning});
    }

    const ce::ext::sampled_rate rate{.sampledrate = 30};
    expect(subject.set(rate).has_value());
    auto read_rate = subject.get<ce::ext::sampled_rate>();
    expect(read_rate.has_value());
    if (read_rate) {
      expect(bool{*read_rate == rate});
    }

    const ce::ext::sequence sequence{.value = "0000000042"};
    expect(subject.set(sequence).has_value());
    auto read_sequence = subject.get<ce::ext::sequence>();
    expect(read_sequence.has_value());
    if (read_sequence) {
      expect(bool{*read_sequence == sequence});
    }

    const ce::ext::dataref dataref{.value = ce::uri_ref{"https://example.test/blob/1"}};
    expect(subject.set(dataref).has_value());
    auto read_dataref = subject.get<ce::ext::dataref>();
  ...
  "set stores each field under its declared attribute type"_test = [] {
    ...
    ce::event subject = minimal();
    expect(subject.set(ce::ext::sampled_rate{.sampledrate = 30}).has_value());
    expect(subject.set(ce::ext::dataref{.value = ce::uri_ref{"/blob"}}).has_value());
    expect(subject.set(ce::ext::partitioning{.partitionkey = "k"}).has_value());
    ...
    if (reference != nullptr) {
      expect(std::holds_alternative<ce::uri_ref>(*reference));
  ...
  "an absent optional field is nullopt, not an error"_test = [] {
    ce::event subject = minimal();
    expect(subject.set(ce::ext::tracing{.traceparent = "00-a-b-01", .tracestate = {}}).has_value());
    expect(subject.extension("tracestate") == nullptr);

    auto read = subject.get<ce::ext::tracing>();
  ...
  "a nullopt optional removes an attribute already present"_test = [] {
    ...
    ce::event subject = minimal();
    expect(
        subject.set(ce::ext::tracing{.traceparent = "00-a-b-01", .tracestate = "v=1"}).has_value());
    expect(subject.extension("tracestate") != nullptr);
  ...
  "an absent required field is an error naming the attribute"_test = [] {
    ...
    expect(!read.has_value());
    if (!read) {
      expect(read.error().code == ce::errc::missing_required_attribute);
    ...
    if (!partial) {
      expect(partial.error().code == ce::errc::missing_required_attribute);
  ...
  "get reports the first failure and does not report a later one"_test = [] {
    ce::event subject = minimal();
    auto read = subject.get<ce::ext::partitioning>();
    expect(!read.has_value());
    if (!read) {
...
void check_json_type_recovery(std::string_view label) {
  ...
  ce::event subject = minimal();
  const ce::ext::tracing tracing{.traceparent = "00-0af7651916cd43dd-b7ad6b7169203331-01",
                                 .tracestate = "congo=t61rcWkgMzE"};
  expect(subject.set(tracing).has_value()) << label;
  expect(subject.set(ce::ext::sampled_rate{.sampledrate = 30}).has_value()) << label;
  expect(subject.set(ce::ext::dataref{.value = ce::uri_ref{"https://example.test/b"}}).has_value())
      << label;
  ...
  auto read_tracing = decoded->template get<ce::ext::tracing>();
  ...
  auto read_rate = decoded->template get<ce::ext::sampled_rate>();
  ...
  auto read_ref = decoded->template get<ce::ext::dataref>();
...
void check_recovery_failures(std::string_view label) {
  ...
  const auto reject = [&label](std::string_view stored, auto probe) {
    ...
    if (!read) {
      expect(read.error().code == ce::errc::type_mismatch) << label << ": " << stored;
  ...
  if (!mismatched) {
    expect(mismatched.error().code == ce::errc::type_mismatch) << label;
  }
}
...
void check_event_of(std::string_view label) {
  using namespace boost::ut;
  using view = ce::event_of<payload, C>;
