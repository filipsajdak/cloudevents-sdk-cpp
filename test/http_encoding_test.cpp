...
const boost::ut::suite<"percent-encoding-refuses-ill-formed-utf8-on-encode">
  ...
  "an ill-formed sequence is refused rather than escaped"_test = [] {
    for (const auto bad : {truncated, overlong, surrogate}) {
      const auto encoded = ce::http::percent_encoded_values::encode(bad);
      expect(!encoded.has_value()) << "should refuse to encode";
      if (!encoded) {
        expect(encoded.error().code == ce::errc::invalid_utf8);
      }
    }
  };
  ...
  "well-formed values still encode, including the ones the rule targets"_test = [] {
    const auto space = ce::http::percent_encoded_values::encode("a b"sv);
    expect(space.has_value());
    if (space) {
      expect(*space == "a%20b");
    }

    const auto multibyte = ce::http::percent_encoded_values::encode("\xC3\xA9"sv);
    expect(multibyte.has_value()) << "two-byte e-acute is well formed and must survive";

    const auto plain = ce::http::percent_encoded_values::encode("com.example.order"sv);
  ...
  "the two policies agree about what is not encodable"_test = [] {
    expect(!ce::http::percent_encoded_values::encode(truncated).has_value());
    expect(!ce::http::literal_values::encode(truncated).has_value());
  };
  ...
  "decode still refuses what it always refused"_test = [] {
    const auto decoded = ce::http::percent_encoded_values::decode("%C3"sv);
