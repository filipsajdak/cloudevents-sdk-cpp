...
const boost::ut::suite<"headers-adopt-refuses-what-it-cannot-hold"> headers_adopt = [] {
  ...
  "fields that arrived once each are adopted unchanged"_test = [] {
    const ce::raw_headers delivered{
        {"x_specversion", "1.0"},
        {"x_id", "1"},
        {"x_source", "/s"},
    };

    const auto adopted = ce::headers::adopt(delivered, ce::name_matching::case_sensitive);
  ...
  "an empty set adopts to an empty set"_test = [] {
    const auto adopted = ce::headers::adopt(ce::raw_headers{}, ce::name_matching::case_sensitive);
    expect(adopted.has_value());
    if (adopted) {
  ...
  "the invariant type offers no way to add a second field of a name"_test = [] {
    static_assert(!has_add<ce::headers>,
    ...
    static_assert(has_add<ce::raw_headers>);
    expect(true);
  };
...
const boost::ut::suite<"headers-adopt-refuses-a-repeated-attribute"> headers_repeated = [] {
  ...
  "a repeated field name is refused, naming the field"_test = [] {
    ...
    expect(!adopted.has_value());
    if (!adopted) {
      expect(adopted.error().code == ce::errc::invalid_argument);
  ...
  "case folding follows the binding's rule"_test = [] {
    const ce::raw_headers mixed{
        {"x_id", "first"},
        {"X_ID", "second"},
    };

    const auto strict = ce::headers::adopt(mixed, ce::name_matching::case_sensitive);
    expect(strict.has_value()) << "byte-exact names make these two different fields";

    const auto lenient = ce::headers::adopt(mixed, ce::name_matching::case_insensitive);
    expect(!lenient.has_value()) << "case-insensitive names make these one attribute twice";
    if (!lenient) {
      expect(lenient.error().code == ce::errc::invalid_argument);
    }
  };
  ...
  "a decoder refuses the message rather than choosing a value"_test = [] {
    ...
    if (!decoded) {
      expect(decoded.error().code == ce::errc::invalid_argument);
