...
namespace binding = ce::v1::binding;
...
struct exact_traits {
  ...
  [[nodiscard]] static auto encode_value(std::string_view text) -> ce::v1::result<std::string> {
    if (!ce::v1::detail::is_valid_utf8(text)) {
      return ce::v1::fail(ce::v1::errc::invalid_utf8, "attribute value is not well-formed UTF-8");
    }
    return std::string{text};
  }

  [[nodiscard]] static auto decode_value(std::string_view text) -> ce::v1::result<std::string> {
...
struct no_prefix {
  ...
  static auto encode_value(std::string_view) -> ce::v1::result<std::string> { return std::string{}; }
  static auto decode_value(std::string_view) -> ce::v1::result<std::string> { return std::string{}; }
};
...
[[nodiscard]] auto field_names(const ce::v1::headers& fields) -> std::vector<std::string> {
...
const boost::ut::suite<"binding-core-emission-order"> binding_core_emission_order = [] {
  ...
  "attributes are emitted in the order the contract fixes"_test = [] {
    ...
    subject.dataschema = ce::v1::uri{"https://example.test/schema"};
    subject.subject = "s";
    subject.time = *ce::v1::parse_timestamp("2026-09-21T00:00:00Z");
    subject.datacontenttype = "text/plain";
    expect(bool{subject.set_extension("alpha", ce::v1::attribute_value{std::string{"a"}})});
    expect(bool{subject.set_extension("beta", ce::v1::attribute_value{std::int32_t{2}})});
...
const boost::ut::suite<"binding-core-name-case"> binding_core_name_case = [] {
  ...
  "a case-sensitive binding does not lower an attribute name"_test = [] {
    ...
    expect(!subject);
    if (!subject) {
      expect(subject.error().code == ce::v1::errc::invalid_attribute_name);
...
const boost::ut::suite<"binding-core-round-trip"> binding_core_round_trip = [] {
  ...
  "a value the traits refuse fails the write, naming the attribute"_test = [] {
    ...
    expect(!written);
    if (!written) {
      expect(written.error().code == ce::v1::errc::invalid_utf8);
  ...
  "a missing required attribute names the first one absent"_test = [] {
    ...
    expect(!subject);
    if (!subject) {
      expect(subject.error().code == ce::v1::errc::missing_required_attribute);
  ...
  "the body round-trips as bytes or as JSON text"_test = [] {
    ...
    ce::v1::event read_back = base_event();
