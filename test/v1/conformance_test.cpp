...
[[nodiscard]] auto parse_header_block(std::string_view text) -> ce::v1::message {
...
void check_literal_structured(std::string_view label) {
  ...
  if (core) {
    ...
    if (core->time) {
      ...
      expect(ce::v1::to_string(*core->time) == "2018-04-05T17:31:00Z") << label;
    }
...
void check_elided_base64(std::string_view label) {
  ...
  if (!single) {
    expect(single.error().code == ce::v1::errc::invalid_base64) << label << ": json-01";
  }

  auto batch = format::decode_batch(read_fixture("json-09-batch.json"));
  expect(!batch.has_value()) << label << ": json-09 should not decode";
  if (!batch) {
    expect(batch.error().code == ce::v1::errc::invalid_base64) << label << ": json-09";
  }
...
void check_literal_binary_headers(std::string_view label) {
  ...
  for (const auto name : {"json-04-binary-headers.http"sv, "json-06-binary-headers.http"sv,
    ...
    const ce::v1::message request = parse_header_block(read_fixture(name));
    ...
    expect(ce::v1::http::detect_content_mode(request) == ce::v1::content_mode::binary_mode)
        << label << ": " << name;
...
void check_media_type_examples() {
  ...
  const ce::v1::message structured = parse_header_block(read_fixture("http-03-structured-content-type.http"));
  expect(ce::v1::http::detect_content_mode(structured) == ce::v1::content_mode::structured);

  const ce::v1::message batched = parse_header_block(read_fixture("http-06-batch-content-type.http"));
  expect(ce::v1::http::detect_content_mode(batched) == ce::v1::content_mode::batched);
...
void check_elided_exchanges() {
  ...
  const ce::v1::message binary_request = parse_header_block(read_fixture("http-01-binary-request.http"));
  ...
  expect(ce::v1::http::detect_content_mode(binary_request) == ce::v1::content_mode::binary_mode);

  const ce::v1::message structured_request =
  ...
  const ce::v1::message batch_request = parse_header_block(read_fixture("http-07-batch-request.http"));
  expect(ce::v1::http::detect_content_mode(batch_request) == ce::v1::content_mode::batched);
