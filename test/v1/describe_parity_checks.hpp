...
template <ce::v1::described T>
void check(const expectation& expected) {
  using namespace boost::ut;

  expect(ce::v1::field_count<T> == expected.wire_names.size())
      << expected.label << ": field_count";

  const auto names = ce::v1::field_names<T>();
  ...
  ce::v1::for_each_field(object, [&visited](std::string_view visited_name, auto&) {
  ...
  ce::v1::for_each_field(const_object, [&const_visited](std::string_view visited_name, const auto&) {
