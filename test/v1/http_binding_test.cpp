...
// both operands when an expectation fails, and ce::v1::uri, ce::v1::uri_ref, their
// std::optional forms and ce::v1::event have no operator<<, so the unwrapped form
// does not compile.
...
[[nodiscard]] auto minimal_binary_message() -> ce::v1::message {
...
void check_to_message_modes(std::string_view label) {
  ...
  auto structured = ce::v1::http::to_message<C>(subject, ce::v1::content_mode::structured);
  ...
  if (structured) {
    ...
    if (declared != nullptr) {
      expect(*declared == ce::v1::json::content_type) << label << ": " << *declared;
    }
    ...
    expect(ce::v1::http::detect_content_mode(*structured) == ce::v1::content_mode::structured) << label;
  }

  auto binary = ce::v1::http::to_message<C>(subject, ce::v1::content_mode::binary_mode);
  ...
  if (binary) {
    ...
    expect(ce::v1::http::detect_content_mode(*binary) == ce::v1::content_mode::binary_mode) << label;
  }
  ...
  invalid.id.clear();
  for (const auto mode : {ce::v1::content_mode::binary_mode, ce::v1::content_mode::structured}) {
    auto refused = ce::v1::http::to_message<C>(invalid, mode);
    expect(!refused.has_value()) << label;
    if (!refused) {
      expect(refused.error().code == ce::v1::errc::missing_required_attribute) << label;
    }
  }
  ...
  // this event is wrong.
  auto wrong_entry_point = ce::v1::http::to_message<C>(subject, ce::v1::content_mode::batched);
  expect(!wrong_entry_point.has_value()) << label;
  if (!wrong_entry_point) {
    expect(wrong_entry_point.error().code == ce::v1::errc::invalid_argument)
        << label << ": " << ce::v1::to_string_view(wrong_entry_point.error().code);
  }
  // The same event through the batch entry point does succeed, so the refusal is
  // about the entry point and not about the event.
  const std::vector<ce::v1::event> one{subject};
  expect(ce::v1::http::to_batch_message<C>(std::span<const ce::v1::event>{one}).has_value()) << label;
}
...
void check_from_message_roundtrip(std::string_view label) {
  ...
  for (const auto mode : {ce::v1::content_mode::binary_mode, ce::v1::content_mode::structured}) {
    auto request = ce::v1::http::to_message<C>(subject, mode);
    expect(request.has_value()) << label;
    if (!request) {
      continue;
    }
    auto back = ce::v1::http::from_message<C>(*request);
    ...
    if (back->time) {
      expect(ce::v1::to_string(*back->time) == "2018-04-05T17:31:00Z"sv) << label;
    }
  ...
  static_assert(std::is_same_v<decltype(ce::v1::http::from_message<C>(std::declval<const ce::v1::message&>())),
                               ce::v1::result<ce::v1::event>>);

  // A malformed structured body fails as a parse error rather than half-decoding.
  ce::v1::message broken;
  broken.header_fields.set("Content-Type", std::string{ce::v1::json::content_type});
  broken.body = ce::v1::http::detail::to_bytes("{");
  auto refused = ce::v1::http::from_message<C>(broken);
  expect(!refused.has_value()) << label;
  if (!refused) {
  ...
  auto missing = ce::v1::http::from_message<C>(incomplete);
  expect(!missing.has_value()) << label;
  if (!missing) {
    expect(missing.error().code == ce::v1::errc::missing_required_attribute) << label;
  ...
  ce::v1::message future = minimal_binary_message();
  future.header_fields.set("ce-specversion", "0.3");
  auto unsupported = ce::v1::http::from_message<C>(future);
  expect(!unsupported.has_value()) << label;
  if (!unsupported) {
    expect(unsupported.error().code == ce::v1::errc::unsupported_spec_version) << label;
  }
  ...
  auto decoded = ce::v1::http::from_message<C>(mixed_case);
  expect(decoded.has_value()) << label;
  if (decoded) {
    expect(decoded->id == "7") << label;
    expect(decoded->type == "t") << label;
    expect(bool{decoded->source == ce::v1::uri_ref{"/s"}}) << label;
  }
}
...
void check_batched_variants(std::string_view label) {
  ...
  const std::vector<ce::v1::event> many{base_event(), base_event()};
  auto request = ce::v1::http::to_batch_message<C>(std::span<const ce::v1::event>{many});
  expect(request.has_value()) << label;
  if (!request) {
    return;
  }
  ...
  expect(declared != nullptr) << label;
  if (declared != nullptr) {
    expect(*declared == ce::v1::json::batch_content_type) << label << ": " << *declared;
  }
  expect(ce::v1::http::detect_content_mode(*request) == ce::v1::content_mode::batched) << label;

  auto back = ce::v1::http::from_batch_message<C>(*request);
  expect(back.has_value()) << label;
  if (back) {
  ...
  auto empty_request = ce::v1::http::to_batch_message<C>(std::span<const ce::v1::event>{none});
  expect(empty_request.has_value()) << label;
  if (empty_request) {
    auto empty_back = ce::v1::http::from_batch_message<C>(*empty_request);
  ...
  auto structured = ce::v1::http::to_message<C>(base_event(), ce::v1::content_mode::structured);
  expect(structured.has_value()) << label;
  if (structured) {
    auto not_a_batch = ce::v1::http::from_batch_message<C>(*structured);
    expect(!not_a_batch.has_value()) << label;
    if (!not_a_batch) {
      expect(not_a_batch.error().code == ce::v1::errc::not_a_cloudevent) << label;
    }
  }
  ...
  auto wrong_entry_point = ce::v1::http::from_message<C>(*request);
  expect(!wrong_entry_point.has_value()) << label;
  if (!wrong_entry_point) {
    expect(wrong_entry_point.error().code == ce::v1::errc::invalid_argument)
        << label << ": " << ce::v1::to_string_view(wrong_entry_point.error().code);
...
void check_detection_of_produced_messages(std::string_view label) {
  ...
  auto binary = ce::v1::http::to_message<C>(base_event(), ce::v1::content_mode::binary_mode);
  ...
  const std::vector<ce::v1::event> many{base_event()};
  auto batched = ce::v1::http::to_batch_message<C>(std::span<const ce::v1::event>{many});
  expect(batched.has_value()) << label;
  if (batched) {
    expect(ce::v1::http::detect_content_mode(*batched) == ce::v1::content_mode::batched) << label;
  }
}
...
void check_binary_mode_ce_headers(std::string_view label) {
  ...
  auto request = ce::v1::http::to_message<C>(subject, ce::v1::content_mode::binary_mode);
  expect(request.has_value()) << label;
  if (!request) {
    return;
  }
  ...
  for (const auto& [name, value] : request->header_fields) {
    if (!ce::v1::http::detail::starts_with_ignoring_case(name, "ce-")) {
      continue;
    }
  ...
  auto bare = ce::v1::http::to_message<C>(base_event(), ce::v1::content_mode::binary_mode);
...
void check_binary_mode_body(std::string_view label) {
  ...
  ce::v1::event with_bytes = base_event();
  with_bytes.data = octets;
  auto from_bytes = ce::v1::http::to_message<C>(with_bytes, ce::v1::content_mode::binary_mode);
  expect(from_bytes.has_value()) << label;
  if (from_bytes) {
    expect(from_bytes->body == octets) << label;
    auto back = ce::v1::http::from_message<C>(*from_bytes);
    expect(back.has_value()) << label;
    if (back) {
      expect(std::holds_alternative<ce::v1::binary>(back->data)) << label;
      if (std::holds_alternative<ce::v1::binary>(back->data)) {
        expect(std::get<ce::v1::binary>(back->data) == octets) << label;
      }
    }
  }

  // Text: the same bytes, not a JSON string with escapes added.
  ce::v1::event with_text = base_event();
  with_text.datacontenttype = "text/plain";
  with_text.data = std::string{R"(hello "world")"};
  auto from_text = ce::v1::http::to_message<C>(with_text, ce::v1::content_mode::binary_mode);
  expect(from_text.has_value()) << label;
  if (from_text) {
    expect(from_text->body == ce::v1::http::detail::to_bytes(R"(hello "world")")) << label;
    expect(ce::v1::http::detail::to_text(from_text->body) == R"(hello "world")"sv) << label;
  }

  // Pre-serialized JSON: the raw text, byte for byte, with no reformatting.
  constexpr auto raw = R"({"k":1,"list":[1,2]})"sv;
  ce::v1::event with_json = base_event();
  with_json.datacontenttype = "application/json";
  with_json.data = ce::v1::json_text{.raw = std::string{raw}};
  auto from_json = ce::v1::http::to_message<C>(with_json, ce::v1::content_mode::binary_mode);
  expect(from_json.has_value()) << label;
  if (from_json) {
    expect(ce::v1::http::detail::to_text(from_json->body) == raw) << label;
    auto back = ce::v1::http::from_message<C>(*from_json);
    expect(back.has_value()) << label;
    if (back && std::holds_alternative<ce::v1::json_text>(back->data)) {
      expect(std::get<ce::v1::json_text>(back->data).raw == raw) << label;
    }
  }
  ...
  if (bare) {
    ...
    auto back = ce::v1::http::from_message<C>(*bare);
    expect(back.has_value()) << label;
    if (back) {
...
void check_percent_encoding_roundtrip(std::string_view label) {
  ...
  ce::v1::event plain = base_event();
  plain.subject = "com.example/path?a=1&b=2";
  auto plain_request = ce::v1::http::to_message<C>(plain, ce::v1::content_mode::binary_mode);
...
void check_percent_decoding(std::string_view label) {
  ...
  for (const auto& [encoded, decoded] : accepted) {
    ce::v1::message request = minimal_binary_message();
  ...
  for (const auto broken : {"abc%4"sv, "abc%"sv, "%"sv, "%ZZ"sv, "%4Z"sv, "%G0"sv}) {
    ...
    auto refused = ce::v1::http::from_message<C>(request);
    expect(!refused.has_value()) << label << ": should reject " << broken;
    if (!refused) {
      expect(refused.error().code == ce::v1::errc::parse_error)
          << label << ": " << broken << " gave " << ce::v1::to_string_view(refused.error().code);
    }
  }
}
...
void check_invalid_utf8_rejected(std::string_view label) {
  ...
  for (const auto& [encoded, why] : ill_formed) {
    ...
    if (!refused) {
      expect(refused.error().code == ce::v1::errc::invalid_utf8)
          << label << ": " << why << " gave " << ce::v1::to_string_view(refused.error().code);
  ...
  ce::v1::message extension_request = minimal_binary_message();
  extension_request.header_fields.set("ce-trace", "%C0%80");
  auto refused_extension = ce::v1::http::from_message<C>(extension_request);
  expect(!refused_extension.has_value()) << label;
  if (!refused_extension) {
    expect(refused_extension.error().code == ce::v1::errc::invalid_utf8) << label;
  }
  ...
  ce::v1::message good = minimal_binary_message();
  good.header_fields.set("ce-subject", "%C3%BC%E6%97%A5%F0%9F%98%80");
  auto accepted = ce::v1::http::from_message<C>(good);
...
void check_extension_string_type(std::string_view label) {
  ...
  expect(subject.set_extension("relref", ce::v1::uri_ref{"/relative"}).has_value()) << label;
...
void check_not_a_cloudevent(std::string_view label) {
  ...
  for (const auto& [media_type, body] : passers_by) {
    ...
    request.body = ce::v1::http::detail::to_bytes(body);
    auto refused = ce::v1::http::from_message<C>(request);
    expect(!refused.has_value()) << label << ": " << media_type;
    if (!refused) {
      expect(refused.error().code == ce::v1::errc::not_a_cloudevent)
          << label << ": " << media_type << " gave " << ce::v1::to_string_view(refused.error().code);
    }
  }
  ...
  ce::v1::message bare;
  auto bare_refused = ce::v1::http::from_message<C>(bare);
  expect(!bare_refused.has_value()) << label;
  if (!bare_refused) {
    expect(bare_refused.error().code == ce::v1::errc::not_a_cloudevent) << label;
  }
  // Even carrying unrelated headers, as long as none of them is ce-specversion.
  ce::v1::message unrelated;
  unrelated.header_fields.set("Authorization", "Bearer x");
  unrelated.header_fields.set("X-Request-Id", "42");
  auto unrelated_refused = ce::v1::http::from_message<C>(unrelated);
  expect(!unrelated_refused.has_value()) << label;
  if (!unrelated_refused) {
    expect(unrelated_refused.error().code == ce::v1::errc::not_a_cloudevent) << label;
  }
  ...
  auto broken_binary = ce::v1::http::from_message<C>(missing_id);
  expect(!broken_binary.has_value()) << label;
  if (!broken_binary) {
    expect(broken_binary.error().code == ce::v1::errc::missing_required_attribute) << label;
    expect(broken_binary.error().code != ce::v1::errc::not_a_cloudevent) << label;
  }

  ce::v1::message broken_structured;
  broken_structured.header_fields.set("Content-Type", std::string{ce::v1::json::content_type});
  broken_structured.body = ce::v1::http::detail::to_bytes(R"({"specversion":)");
  auto structured_error = ce::v1::http::from_message<C>(broken_structured);
  expect(!structured_error.has_value()) << label;
  if (!structured_error) {
    expect(structured_error.error().code == ce::v1::errc::parse_error) << label;
    expect(structured_error.error().code != ce::v1::errc::not_a_cloudevent) << label;
  }

  ce::v1::message bad_time = minimal_binary_message();
  bad_time.header_fields.set("ce-time", "not-a-timestamp");
  auto time_error = ce::v1::http::from_message<C>(bad_time);
  expect(!time_error.has_value()) << label;
  if (!time_error) {
    expect(time_error.error().code != ce::v1::errc::not_a_cloudevent) << label;
  }
  ...
  good.body = ce::v1::http::detail::to_bytes("hello");
  auto accepted = ce::v1::http::from_message<C>(good);
  expect(accepted.has_value()) << label;
  if (accepted) {
...
void check_spec_examples(std::string_view label) {
  ...
  binary_request.body = ce::v1::http::detail::to_bytes(R"({"much":"wow"})");

  expect(ce::v1::http::detect_content_mode(binary_request) == ce::v1::content_mode::binary_mode) << label;
  auto from_binary = ce::v1::http::from_message<C>(binary_request);
  ...
  expect(bool{from_binary->source == ce::v1::uri_ref{"/mycontext/subcontext"}}) << label;
  expect(bool{from_binary->datacontenttype == std::optional<std::string>{"application/json"}})
      << label;
  expect(from_binary->time.has_value()) << label;
  if (from_binary->time) {
    expect(ce::v1::to_string(*from_binary->time) == "2018-04-05T03:56:24Z"sv) << label;
  }
  expect(from_binary->extensions.size() == 2U) << label;
  expect(std::holds_alternative<ce::v1::json_text>(from_binary->data)) << label;
  if (std::holds_alternative<ce::v1::json_text>(from_binary->data)) {
    expect(std::get<ce::v1::json_text>(from_binary->data).raw == R"({"much":"wow"})"sv) << label;
  }

  // Sending the decoded event back out reproduces the example's headers, so the
  // two directions agree on the same wire form.
  auto re_sent = ce::v1::http::to_message<C>(*from_binary, ce::v1::content_mode::binary_mode);
  ...
  if (re_sent) {
    ...
    expect(ce::v1::http::detail::to_text(re_sent->body) == R"({"much":"wow"})"sv) << label;
  }

  // Structured mode, section 3.2.4: the same event, wholly in the body.
  ce::v1::message structured_request;
  structured_request.header_fields.set("content-type",
                                       "application/cloudevents+json; charset=UTF-8");
  structured_request.body = ce::v1::http::detail::to_bytes(spec_structured_body);
  expect(ce::v1::http::detect_content_mode(structured_request) == ce::v1::content_mode::structured)
      << label;
  auto from_structured = ce::v1::http::from_message<C>(structured_request);
  ...
  if (from_structured) {
    ...
    expect(bool{from_structured->source == ce::v1::uri_ref{"/mycontext/subcontext"}}) << label;
    ...
    ce::v1::event aligned = *from_binary;
  ...
  ce::v1::message batch_request;
  batch_request.header_fields.set("content-type",
                                  "application/cloudevents-batch+json; charset=UTF-8");
  batch_request.body = ce::v1::http::detail::to_bytes(batch_body);
  expect(ce::v1::http::detect_content_mode(batch_request) == ce::v1::content_mode::batched) << label;
  auto from_batch = ce::v1::http::from_batch_message<C>(batch_request);
...
void check_ce_header_name_grammar(std::string_view label) {
  ...
  for (const std::string_view name : {
    ...
    auto decoded = ce::v1::http::from_message<C>(request);
    expect(!decoded.has_value()) << label << ": should reject header " << name;
    if (!decoded) {
      expect(decoded.error().code == ce::v1::errc::invalid_attribute_name) << label << ": " << name;
    }
  }
  ...
  // would not validate; the presence check alone does not catch it.
  {
    ...
    expect(!ce::v1::http::from_message<C>(empty_type).has_value()) << label << ": empty ce-type";
  }
  ...
  if (decoded) {
    ...
    expect(ce::v1::http::to_message<C>(*decoded, ce::v1::content_mode::binary_mode).has_value()) << label;
  }
}
...
const boost::ut::suite<"message-type-shape"> message_type_shape = [] {
  ...
  "message is an aggregate of headers and a binary body"_test = [] {
    ...
    static_assert(std::is_aggregate_v<ce::v1::message>);
    static_assert(std::is_same_v<decltype(ce::v1::message::header_fields), ce::v1::headers>);
    static_assert(std::is_same_v<decltype(ce::v1::message::body), ce::v1::binary>);
    static_assert(std::is_same_v<ce::v1::binary, std::vector<std::byte>>);
    static_assert(std::is_default_constructible_v<ce::v1::message>);
    static_assert(std::is_copy_constructible_v<ce::v1::message>);
    static_assert(std::is_move_constructible_v<ce::v1::message>);
    expect(true);
  };

  "a default message is empty and two of them compare equal"_test = [] {
    const ce::v1::message left;
    const ce::v1::message right;
    expect(left.header_fields.empty());
    expect(left.body.empty());
    expect(bool{left == right});

    const ce::v1::message filled{.header_fields = {}, .body = ce::v1::http::detail::to_bytes("x")};
  ...
  "the binding names no HTTP library type"_test = [] {
    ...
    static_assert(std::is_same_v<ce::v1::headers::entry, std::pair<std::string, std::string>>);
    ce::v1::message request;
    request.header_fields.add("Content-Type", "text/plain");
    request.body = ce::v1::http::detail::to_bytes("payload");
    expect(request.header_fields.size() == 1U);
    expect(ce::v1::http::detail::to_text(request.body) == "payload"sv);
  };
};
...
const boost::ut::suite<"headers-ordered-multimap"> headers_ordered_multimap = [] {
  ...
  "add keeps repeated field names, in order"_test = [] {
    ...
    for (const auto& [name, value] : fields) {
      if (ce::v1::detail::iequals(name, "set-cookie")) {
  ...
  "set replaces every header of that name"_test = [] {
    ...
    for (const auto& [name, value] : fields) {
      if (ce::v1::detail::iequals(name, "accept")) {
        ++accept_count;
...
const boost::ut::suite<"to-message-modes"> to_message_modes = [] {
  ...
  "the send-side entry points have the declared signatures"_test = [] {
    static_assert(
        std::is_same_v<decltype(ce::v1::http::to_message<nlohmann_codec>(
                           std::declval<const ce::v1::event&>(), ce::v1::content_mode::binary_mode)),
                       ce::v1::result<ce::v1::message>>);
    static_assert(
        std::is_same_v<decltype(ce::v1::http::to_message<mini_codec>(
                           std::declval<const ce::v1::event&>(), ce::v1::content_mode::structured)),
                       ce::v1::result<ce::v1::message>>);
    expect(true);
  };
};
...
const boost::ut::suite<"batched-variants"> batched_variants = [] {
  ...
  "the batch entry points have the declared signatures"_test = [] {
    static_assert(std::is_same_v<decltype(ce::v1::http::to_batch_message<nlohmann_codec>(
                                     std::declval<std::span<const ce::v1::event>>())),
                                 ce::v1::result<ce::v1::message>>);
    static_assert(std::is_same_v<decltype(ce::v1::http::from_batch_message<mini_codec>(
                                     std::declval<const ce::v1::message&>())),
                                 ce::v1::result<std::vector<ce::v1::event>>>);
    expect(true);
  };
};
...
const boost::ut::suite<"content-mode-detection"> content_mode_detection = [] {
  ...
  "the batch prefix is tested before the cloudevents prefix"_test = [] {
    ...
    expect(ce::v1::json::batch_content_type.starts_with("application/cloudevents"sv))
        << "the premise of the ordering rule no longer holds";

    ce::v1::message request;
    request.header_fields.set("Content-Type", std::string{ce::v1::json::batch_content_type});
    expect(ce::v1::http::detect_content_mode(request) == ce::v1::content_mode::batched);
    expect(ce::v1::http::detect_content_mode(request) != ce::v1::content_mode::structured);
  };
  ...
  "no Content-Type at all is the binary mode"_test = [] {
    ...
    const ce::v1::message bare;
    expect(ce::v1::http::detect_content_mode(bare) == ce::v1::content_mode::binary_mode);

    ce::v1::message with_other_headers;
    with_other_headers.header_fields.set("ce-specversion", "1.0");
    with_other_headers.header_fields.set("Accept", "*/*");
    expect(ce::v1::http::detect_content_mode(with_other_headers) == ce::v1::content_mode::binary_mode);
  };

  "each media type selects its mode"_test = [] {
    const std::pair<std::string_view, ce::v1::content_mode> cases[] = {
        {"application/cloudevents-batch+json"sv, ce::v1::content_mode::batched},
        {"application/cloudevents-batch+json; charset=utf-8"sv, ce::v1::content_mode::batched},
        {"APPLICATION/CLOUDEVENTS-BATCH+JSON"sv, ce::v1::content_mode::batched},
        {"application/cloudevents+json"sv, ce::v1::content_mode::structured},
        // A parameterised media type stays in the structured branch, which is
        // why the test is a prefix test and not an equality test.
        {"application/cloudevents+json; charset=utf-8"sv, ce::v1::content_mode::structured},
        {"Application/CloudEvents+JSON"sv, ce::v1::content_mode::structured},
        {"application/cloudevents+xml"sv, ce::v1::content_mode::structured},
        {"application/json"sv, ce::v1::content_mode::binary_mode},
        {"text/plain"sv, ce::v1::content_mode::binary_mode},
        {"application/octet-stream"sv, ce::v1::content_mode::binary_mode},
        {""sv, ce::v1::content_mode::binary_mode},
        // Near misses: the prefix has to be a prefix.
        {"x-application/cloudevents+json"sv, ce::v1::content_mode::binary_mode},
        {"application/cloudevent+json"sv, ce::v1::content_mode::binary_mode},
    };
    for (const auto& [media_type, mode] : cases) {
      ce::v1::message request;
      request.header_fields.set("Content-Type", std::string{media_type});
      expect(ce::v1::http::detect_content_mode(request) == mode) << media_type;
    }
  };
  ...
  "the Content-Type header name is matched case-insensitively"_test = [] {
    for (const auto spelling : {"Content-Type"sv, "content-type"sv, "CONTENT-TYPE"sv}) {
      ce::v1::message request;
      request.header_fields.set(std::string{spelling}, std::string{ce::v1::json::content_type});
      expect(ce::v1::http::detect_content_mode(request) == ce::v1::content_mode::structured) << spelling;
    }
  };
...
const boost::ut::suite<"header-value-percent-encoding"> header_value_percent_encoding = [] {
  ...
  "the three specials and every non-ASCII octet are escaped"_test = [] {
    ...
    expect(ce::v1::http::detail::percent_encode(" ") == "%20"sv);
    expect(ce::v1::http::detail::percent_encode("\"") == "%22"sv);
    expect(ce::v1::http::detail::percent_encode("%") == "%25"sv);
    expect(ce::v1::http::detail::percent_encode(std::string{two_byte_utf8}) == "%C3%BC"sv);
    expect(ce::v1::http::detail::percent_encode(std::string{three_byte_utf8}) == "%E6%97%A5"sv);
    expect(ce::v1::http::detail::percent_encode(std::string{four_byte_utf8}) == "%F0%9F%98%80"sv);
    // Control characters are outside printable ASCII and go the same way.
    expect(ce::v1::http::detail::percent_encode("\n") == "%0A"sv);
    expect(ce::v1::http::detail::percent_encode("\t") == "%09"sv);
    expect(ce::v1::http::detail::percent_encode(std::string{'\x7F'}) == "%7F"sv);
    // The hex digits are upper case, so two senders produce the same bytes.
    expect(ce::v1::http::detail::percent_encode(std::string{'\xAB'}) == "%AB"sv);

    // Printable ASCII other than the quote and the percent is left alone.
    constexpr auto untouched = "!#$&'()*+,-./0123456789:;<=>?@ABCXYZ[\\]^_`abcxyz{|}~"sv;
    expect(ce::v1::http::detail::percent_encode(untouched) == untouched);

    // needs_escape is the single rule both of those follow.
    static_assert(ce::v1::http::detail::needs_escape(' '));
    static_assert(ce::v1::http::detail::needs_escape('"'));
    static_assert(ce::v1::http::detail::needs_escape('%'));
    static_assert(ce::v1::http::detail::needs_escape(0x20));
    static_assert(ce::v1::http::detail::needs_escape(0x7F));
    static_assert(ce::v1::http::detail::needs_escape(0x80));
    static_assert(!ce::v1::http::detail::needs_escape('!'));
    static_assert(!ce::v1::http::detail::needs_escape('~'));
    static_assert(!ce::v1::http::detail::needs_escape('A'));
  };
...
const boost::ut::suite<"header-value-percent-decoding"> header_value_percent_decoding = [] {
  ...
  "any octet may arrive escaped, including ones that needed no escape"_test = [] {
    ...
    for (const auto& [encoded, decoded] : cases) {
      auto result = ce::v1::http::detail::percent_decode(encoded);
    ...
    auto lower = ce::v1::http::detail::percent_decode("%c3%bc");
  ...
  "a truncated or non-hexadecimal escape is a parse error"_test = [] {
    for (const auto broken : {"%"sv, "%4"sv, "abc%4"sv, "abc%"sv, "%ZZ"sv, "%4Z"sv, "%Z4"sv,
                              "%G0"sv, "a%-1b"sv}) {
      auto refused = ce::v1::http::detail::percent_decode(broken);
      expect(!refused.has_value()) << "should reject " << broken;
      if (!refused) {
        expect(refused.error().code == ce::v1::errc::parse_error)
            << broken << " gave " << ce::v1::to_string_view(refused.error().code);
      }
    }
  };
...
const boost::ut::suite<"header-value-invalid-utf8"> header_value_invalid_utf8 = [] {
  ...
  "ill-formed UTF-8 is rejected in every one of its forms"_test = [] {
    ...
    expect(ce::v1::http::detail::is_valid_utf8("plain ascii"));
    expect(ce::v1::http::detail::is_valid_utf8(two_byte_utf8));
    expect(ce::v1::http::detail::is_valid_utf8(three_byte_utf8));
    expect(ce::v1::http::detail::is_valid_utf8(four_byte_utf8));
    expect(ce::v1::http::detail::is_valid_utf8(""sv));

    expect(!ce::v1::http::detail::is_valid_utf8("\xC0\x80"sv));          // overlong NUL
    expect(!ce::v1::http::detail::is_valid_utf8("\xC1\xBF"sv));          // overlong
    expect(!ce::v1::http::detail::is_valid_utf8("\xE0\x80\xAF"sv));      // overlong solidus
    expect(!ce::v1::http::detail::is_valid_utf8("\xF0\x80\x80\xAF"sv));  // overlong, four bytes
    expect(!ce::v1::http::detail::is_valid_utf8("\xED\xA0\x80"sv));      // surrogate D800
    expect(!ce::v1::http::detail::is_valid_utf8("\xED\xBF\xBF"sv));      // surrogate DFFF
    expect(!ce::v1::http::detail::is_valid_utf8("\xF5\x80\x80\x80"sv));  // above U+10FFFF
    expect(!ce::v1::http::detail::is_valid_utf8("\xF4\x90\x80\x80"sv));  // above U+10FFFF
    expect(!ce::v1::http::detail::is_valid_utf8("\xE2\x82"sv));          // truncated
    expect(!ce::v1::http::detail::is_valid_utf8("\x80"sv));              // stray continuation
    expect(!ce::v1::http::detail::is_valid_utf8("\xFE"sv));              // never a lead byte
    expect(!ce::v1::http::detail::is_valid_utf8("\xFF"sv));

    // constexpr, so a caller may use it where no runtime check would run.
    static_assert(ce::v1::http::detail::is_valid_utf8("ascii"));
    static_assert(!ce::v1::http::detail::is_valid_utf8("\xC0\x80"));
  };
...
void check_value_policy(std::string_view codec) {
  ...
  auto conformant = ce::v1::http::to_message<Codec>(subject, ce::v1::content_mode::binary_mode);
  ...
  auto literal =
      ce::v1::http::to_message<Codec, ce::v1::http::literal_values>(subject, ce::v1::content_mode::binary_mode);
  ...
  ce::v1::message from_go = minimal_binary_message();
  from_go.header_fields.set("ce-subject", "100%");

  auto strict = ce::v1::http::from_message<Codec>(from_go);
  expect(!strict) << codec;

  auto lenient = ce::v1::http::from_message<Codec, ce::v1::http::literal_values>(from_go);
  ...
  ce::v1::message from_cpp = minimal_binary_message();
  from_cpp.header_fields.set("ce-subject", "a%20b");
  auto as_go_sees_it = ce::v1::http::from_message<Codec, ce::v1::http::literal_values>(from_cpp);
...
void check_literal_refuses_control_characters(std::string_view codec) {
  ...
  for (const std::string& injected : injected_values) {
    ...
    auto written = ce::v1::http::to_message<Codec, ce::v1::http::literal_values>(
        subject, ce::v1::content_mode::binary_mode);
    expect(!written) << codec << " accepted a control character";
    if (!written) {
      expect(written.error().code == ce::v1::errc::invalid_argument) << codec;
    }
  }
  ...
  ce::v1::event subject = base_event();
  subject.subject = "a\r\nX-Evil: 1";
  auto escaped = ce::v1::http::to_message<Codec>(subject, ce::v1::content_mode::binary_mode);
...
const boost::ut::suite<"http-value-policy"> http_value_policy = [] {
  ...
  static_assert(ce::v1::http::value_policy<ce::v1::http::percent_encoded_values>);
  static_assert(ce::v1::http::value_policy<ce::v1::http::literal_values>);
