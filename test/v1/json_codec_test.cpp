...
void check_parse_dump_roundtrip(std::string_view label) {
  ...
  for (const auto bad : {"{"sv, R"({"a":})"sv, "[1,"sv, "tru"sv, ""sv}) {
    ...
    if (!rejected) {
      expect(rejected.error().code == ce::v1::errc::parse_error) << label << " on " << bad;
    }
  }
}
...
void check_value_constructors(std::string_view label) {
  ...
  expect(C::kind_of(C::make_null()) == ce::v1::json::kind::null) << label;
  expect(C::kind_of(C::make_bool(true)) == ce::v1::json::kind::boolean) << label;
  expect(C::kind_of(C::make_int(7)) == ce::v1::json::kind::integer) << label;
  expect(C::kind_of(C::make_double(1.5)) == ce::v1::json::kind::floating) << label;
  expect(C::kind_of(C::make_string("text")) == ce::v1::json::kind::string) << label;
  expect(C::kind_of(C::make_array()) == ce::v1::json::kind::array) << label;
  expect(C::kind_of(C::make_object()) == ce::v1::json::kind::object) << label;
...
void check_set_and_push(std::string_view label) {
  ...
  if (nested != nullptr) {
    expect(C::kind_of(*nested) == ce::v1::json::kind::array) << label;
...
void check_find_and_traversal(std::string_view label) {
  ...
  if (null_member != nullptr) {
    expect(C::kind_of(*null_member) == ce::v1::json::kind::null) << label;
  }
...
void check_kind_and_as_accessors(std::string_view label) {
  ...
  expect(C::kind_of(*document) == ce::v1::json::kind::object) << label;
  ...
  expect(C::kind_of(*integer) == ce::v1::json::kind::integer) << label;
  expect(C::kind_of(*text) == ce::v1::json::kind::string) << label;
  expect(C::kind_of(*array) == ce::v1::json::kind::array) << label;
  expect(C::kind_of(*null_member) == ce::v1::json::kind::null) << label;
  expect(C::kind_of(*boolean) == ce::v1::json::kind::boolean) << label;
  ...
  expect(C::kind_of(*number) == ce::v1::json::kind::floating) << label;
  ...
  if (!wrong_bool) {
    expect(wrong_bool.error().code == ce::v1::errc::type_mismatch) << label;
  }
  const auto wrong_string = C::as_string(*integer);
  expect(!wrong_string.has_value()) << label;
  if (!wrong_string) {
    expect(wrong_string.error().code == ce::v1::errc::type_mismatch) << label;
  }
...
const boost::ut::suite<"json-codec-concept-dom-surface"> codec_concept_dom_surface = [] {
  ...
  "both in-tree codecs satisfy the concept"_test = [] {
    static_assert(ce::v1::json::json_codec<nlohmann_codec>);
  ...
  "the surface is a nested value type plus operations over it"_test = [] {
    ...
    if (inner != nullptr) {
      expect(mini_codec::kind_of(*inner) == ce::v1::json::kind::object);
    }
  };
  ...
  "a type that is not a JSON DOM does not satisfy it"_test = [] {
    static_assert(!ce::v1::json::json_codec<value_only_codec>);
    static_assert(!ce::v1::json::json_codec<int>);
    static_assert(!ce::v1::json::json_codec<nlohmann_codec::value>);
    static_assert(!ce::v1::json::json_codec<no_make_object_codec>);
    expect(true);
  };
};
...
const boost::ut::suite<"json-codec-parse-dump-roundtrip"> codec_parse_dump_roundtrip = [] {
  ...
  "parse returns a result and dump returns a std::string"_test = [] {
    static_assert(std::is_same_v<decltype(nlohmann_codec::parse(""sv)),
                                 ce::v1::result<nlohmann_codec::value>>);
    static_assert(
        std::is_same_v<decltype(mini_codec::parse(""sv)), ce::v1::result<mini_codec::value>>);
    static_assert(
...
const boost::ut::suite<"json-codec-find-and-traversal"> codec_find_and_traversal = [] {
  ...
  "find returns a pointer, so absent is distinguishable from null"_test = [] {
    ...
    static_assert(!ce::v1::json::json_codec<find_by_value_codec>);
    expect(true);
  };
...
const boost::ut::suite<"json-codec-kind-and-as-accessors"> codec_kind_and_as_accessors = [] {
  ...
  "an as_ accessor must return a result"_test = [] {
    static_assert(std::is_same_v<decltype(mini_codec::as_int(
                                 ...
                                 ce::v1::result<std::int64_t>>);
    static_assert(!ce::v1::json::json_codec<unchecked_as_int_codec>);
    expect(true);
  };
...
const boost::ut::suite<"nlohmann-codec-non-throwing-parse"> nlohmann_non_throwing_parse = [] {
  ...
  "malformed text is errc::parse_error, not an exception"_test = [] {
    for (const auto bad : {"{"sv, R"({"a":})"sv, "[1,"sv, "tru"sv, ""sv, R"({"a":1,})"sv,
      ...
      try {
        ...
        rejected = !parsed.has_value() && parsed.error().code == ce::v1::errc::parse_error;
      } catch (...) {
        threw = true;
      }
      ...
      if (!parsed) {
        expect(parsed.error().code == ce::v1::errc::parse_error) << bad;
      }
#endif
    }
  };
  ...
  "a pathologically nested document is a returned error or a value, never a throw"_test = [] {
    ...
    expect(parsed.has_value() || parsed.error().code == ce::v1::errc::parse_error);
  };
...
const boost::ut::suite<"mini-codec-number-parsing-never-throws"> mini_codec_numbers = [] {
  ...
  "malformed and out-of-range numbers are parse errors"_test = [] {
    for (const auto document : {
      ...
      if (!parsed) {
        expect(parsed.error().code == ce::v1::errc::parse_error) << document;
      }
    }
  };
...
const boost::ut::suite<"codecs-decode-surrogate-pairs"> surrogate_pairs = [] {
  ...
  "a lone surrogate is refused rather than encoded"_test = [&escaped] {
    ...
    for (const auto hex : {"D83D"sv, "DE00"sv}) {
      ...
      if (!parsed) {
        expect(parsed.error().code == ce::v1::errc::parse_error);
      }
    }
  };
};
...
void check_integer_range(std::string_view label) {
  ...
  for (const auto document : {R"({"n":9223372036854775808})"sv,
    ...
    if (!parsed) {
      expect(parsed.error().code == ce::v1::errc::parse_error) << label << ": " << document;
      continue;
    }
    ...
    expect(C::kind_of(*member) == ce::v1::json::kind::integer) << label << ": " << document;

    auto held = C::as_int(*member);
    expect(!held.has_value()) << label << ": " << document << " must not yield a value";
    if (!held) {
      expect(held.error().code == ce::v1::errc::out_of_range) << label << ": " << document;
    }
  }
...
void check_number_kinds(std::string_view label) {
  ...
  const auto kind_of_member = [&label](std::string_view document) -> std::optional<ce::v1::json::kind> {
    auto parsed = C::parse(document);
    if (!parsed) {
      return std::nullopt;
    }
  ...
  for (const auto document : {R"({"n":1})"sv, R"({"n":-1})"sv, R"({"n":0})"sv}) {
    ...
    if (kind) {
      expect(*kind == ce::v1::json::kind::integer) << label << ": " << document;
    }
  }
  ...
  for (const auto document : {R"({"n":1.0})"sv, R"({"n":1.5})"sv, R"({"n":1e3})"sv}) {
    ...
    if (kind) {
      ...
      expect(*kind == ce::v1::json::kind::floating) << label << ": " << document;
    }
  }
