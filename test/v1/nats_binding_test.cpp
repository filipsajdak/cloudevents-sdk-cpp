...
void check_round_trip(std::string_view codec) {
  ...
  subject.data = ce::v1::json_text{.raw = "{\"a\":1}"};
  expect(bool{subject.set_extension("alpha", ce::v1::attribute_value{std::string{"a"}})}) << codec;

  auto payload = ce::v1::nats::to_payload<Codec>(subject);
  expect(bool{payload}) << codec;
  if (!payload) {
    return;
  }

  auto read_back = ce::v1::nats::from_payload<Codec>(*payload);
...
void check_payload_is_the_whole_message(std::string_view codec) {
  ...
  static_assert(std::is_same_v<decltype(payload), ce::v1::result<std::string>>);
}
...
void check_every_failure_is_a_parse_error(std::string_view codec) {
  ...
  auto unrelated = ce::v1::nats::from_payload<Codec>(R"({"hello":"world"})"sv);
  expect(!unrelated) << codec;
  if (!unrelated) {
    expect(unrelated.error().code != ce::v1::errc::not_a_cloudevent) << codec;
  }

  auto malformed = ce::v1::nats::from_payload<Codec>("{not json"sv);
  expect(!malformed) << codec;
  if (!malformed) {
    expect(malformed.error().code == ce::v1::errc::parse_error) << codec;
  }

  auto empty = ce::v1::nats::from_payload<Codec>(""sv);
  expect(!empty) << codec;
  if (!empty) {
    expect(empty.error().code != ce::v1::errc::not_a_cloudevent) << codec;
  }
}
...
void check_binary_mode(std::string_view codec) {
  ...
  auto out = ce::v1::nats::to_message<Codec>(subject, ce::v1::content_mode::binary_mode);
  ...
  expect(ce::v1::to_text(out->body) == "hello") << codec;

  auto read_back = ce::v1::nats::from_message<Codec>(*out);
  expect(bool{read_back}) << codec;
  if (read_back) {
    expect(read_back->id == subject.id) << codec;
    expect(bool{read_back->subject == subject.subject}) << codec;
...
void check_mode_detection(std::string_view codec) {
  ...
  ce::v1::message bare;
  expect(ce::v1::nats::detect_content_mode(bare) == ce::v1::content_mode::binary_mode) << codec;

  auto refused = ce::v1::nats::from_message<Codec>(bare);
  expect(!refused) << codec;
  if (!refused) {
    expect(refused.error().code == ce::v1::errc::not_a_cloudevent) << codec;
  }

  ce::v1::message structured;
  structured.header_fields.set("content-type", "application/cloudevents+json; charset=utf-8");
  expect(ce::v1::nats::detect_content_mode(structured) == ce::v1::content_mode::structured) << codec;

  const ce::v1::event subject = base_event();
  auto encoded = ce::v1::nats::to_message<Codec>(subject, ce::v1::content_mode::structured);
  expect(bool{encoded}) << codec;
  if (encoded) {
    expect(encoded->header_fields.find("Content-Type") != nullptr) << codec;
    auto read_back = ce::v1::nats::from_message<Codec>(*encoded);
    expect(bool{read_back}) << codec;
    if (read_back) {
      expect(read_back->id == subject.id) << codec;
    }
  ...
  auto batched = ce::v1::nats::to_message<Codec>(subject, ce::v1::content_mode::batched);
  expect(!batched) << codec;
  if (!batched) {
    expect(batched.error().code == ce::v1::errc::invalid_argument) << codec;
  }
}
...
const boost::ut::suite<"nats-structured-only"> nats_structured_only = [] {
  ...
  static_assert(std::is_invocable_v<decltype(&ce::v1::nats::to_payload<ce::v1::test::mini_codec>),
                                    const ce::v1::event&>);
...
const boost::ut::suite<"nats-payload-is-the-whole-message"> nats_payload_is_the_whole_message = [] {
  ...
  static_assert(!std::is_invocable_v<decltype(&ce::v1::nats::to_payload<ce::v1::test::mini_codec>),
                                     const ce::v1::event&, std::string_view>);
