    ...
    for (const auto& source : sources) {
      for (const auto& type : types) {
        for (int shape = 0; shape < 4; ++shape) {
          ce::v1::event subject{.id = id, .source = ce::v1::uri_ref{source}, .type = type};

          if (index % 2 == 0) {
            subject.subject = "subject-" + std::to_string(index);
          }
          if (index % 3 == 0) {
            subject.dataschema = ce::v1::uri{"https://example.test/schema/" + std::to_string(index)};
          }
          if (index % 5 == 0) {
            if (auto parsed = ce::v1::parse_timestamp("2026-09-20T12:34:56.789-05:30")) {
              subject.time = *parsed;
            }
          }
          if (index % 7 == 0) {
            (void)subject.set(ce::v1::ext::tracing{.traceparent = "00-a-b-01", .tracestate = {}});
          }
          if (index % 11 == 0) {
            (void)subject.set_extension("seq", ce::v1::attribute_value{std::int32_t{42}});
            (void)subject.set_extension("ok", ce::v1::attribute_value{true});
          }
          ...
          switch (shape) {
            ...
            case 1:
              ...
              subject.data = ce::v1::json_text{.raw = R"({"n":)" + std::to_string(index) + "}"};
              break;
            case 2:
            ...
            default:
              ...
              subject.data = ce::v1::binary{std::byte{0x00}, std::byte{0x7F},
...
void check_structured_message_roundtrip(std::string_view label) {
  ...
  for (const auto& [subject, index] : generate()) {
    auto message = ce::v1::http::to_message<C>(subject, ce::v1::content_mode::structured);
    expect(message.has_value()) << label << " #" << index;
    if (!message) {
      continue;
    }
...
void check_batched_roundtrip(std::string_view label) {
  ...
  auto message = ce::v1::http::to_batch_message<C>(std::span<const ce::v1::event>{events});
  expect(message.has_value()) << label;
  if (!message) {
    return;
  }
  auto from_message = ce::v1::http::from_batch_message<C>(*message);
...
const boost::ut::suite<"roundtrip-property"> roundtrip_property = [] {
  ...
  "the generator varies what it claims to vary"_test = [] {
    ...
    for (const auto& [subject, index] : events) {
      ...
      with_json += std::holds_alternative<ce::v1::json_text>(subject.data) ? 1 : 0;
      with_text += std::holds_alternative<std::string>(subject.data) ? 1 : 0;
      with_binary += std::holds_alternative<ce::v1::binary>(subject.data) ? 1 : 0;
