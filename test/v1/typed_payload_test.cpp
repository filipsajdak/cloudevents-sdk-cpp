...
template<class E>
concept carries_typed_extension_member = requires(E subject, ce::v1::ext::tracing value) {
  subject.set(value);
  subject.template get<ce::v1::ext::tracing>();
};
...
void check_payload_roundtrip(std::string_view label) {
  ...
  expect(ce::v1::set_data<reading, C>(subject, sent).has_value()) << label;
  ...
  if (subject.datacontenttype) {
    expect(ce::v1::is_json_content_type(*subject.datacontenttype)) << label;
  }
  ...
  expect(std::holds_alternative<ce::v1::json_text>(subject.data)) << label;

  auto read = ce::v1::data_as<reading, C>(subject);
...
void check_payload_through_the_wire(std::string_view label) {
  ...
  auto read = ce::v1::data_as<reading, C>(*decoded);
  expect(read.has_value()) << label;
  if (read) {
    expect(same(*read, sent)) << label;
  }
...
void check_payload_absent_members(std::string_view label) {
  ...
  subject.data = ce::v1::json_text{.raw = R"({"sensor":"s-1","celsius":3})"};
  ...
  ce::v1::event explicit_null = minimal();
  explicit_null.datacontenttype = "application/json";
  explicit_null.data = ce::v1::json_text{.raw = R"({"sensor":"s","celsius":1,"note":null})"};
  auto null_read = ce::v1::data_as<reading, C>(explicit_null);
...
void check_payload_failures(std::string_view label) {
  ...
  // No payload at all.
  auto empty = ce::v1::data_as<reading, C>(minimal());
  expect(!empty.has_value()) << label;
  if (!empty) {
    expect(empty.error().code == ce::v1::errc::missing_required_attribute) << label;
    expect(empty.error().where == "data") << label;
  }

  // Bytes are refused rather than guessed at.
  ce::v1::event binary = minimal();
  binary.data = ce::v1::binary{std::byte{0x00}, std::byte{0x01}};
  auto from_binary = ce::v1::data_as<reading, C>(binary);
  expect(!from_binary.has_value()) << label;
  if (!from_binary) {
    expect(from_binary.error().code == ce::v1::errc::type_mismatch) << label;
  }
  ...
  ce::v1::event text = minimal();
  text.datacontenttype = "text/plain";
  text.data = std::string{R"({"sensor":"s-1","celsius":1})"};
  auto from_text = ce::v1::data_as<reading, C>(text);
  expect(!from_text.has_value()) << label;
  if (!from_text) {
    expect(from_text.error().code == ce::v1::errc::type_mismatch) << label;
  }
  text.datacontenttype = "application/json";
  expect(ce::v1::data_as<reading, C>(text).has_value()) << label;
  ...
  wrong.data = ce::v1::json_text{.raw = R"({"sensor":7,"celsius":1})"};
  auto mistyped = ce::v1::data_as<reading, C>(wrong);
  expect(!mistyped.has_value()) << label;
  if (!mistyped) {
    expect(mistyped.error().code == ce::v1::errc::type_mismatch) << label;
    expect(mistyped.error().where == "sensor") << label;
  }

  // A JSON document that is not an object cannot be a described struct.
  ce::v1::event array = minimal();
  array.datacontenttype = "application/json";
  array.data = ce::v1::json_text{.raw = "[1,2,3]"};
  auto from_array = ce::v1::data_as<reading, C>(array);
  expect(!from_array.has_value()) << label;
  if (!from_array) {
    expect(from_array.error().code == ce::v1::errc::type_mismatch) << label;
  }

  // Unparseable text is a parse error, distinct from a type mismatch.
  ce::v1::event broken = minimal();
  broken.datacontenttype = "application/json";
  broken.data = ce::v1::json_text{.raw = R"({"sensor":)"};
  auto unparsed = ce::v1::data_as<reading, C>(broken);
  expect(!unparsed.has_value()) << label;
  if (!unparsed) {
    expect(unparsed.error().code == ce::v1::errc::parse_error) << label;
    expect(unparsed.error().code != ce::v1::errc::type_mismatch) << label;
  }
  ...
  huge.data = ce::v1::json_text{.raw = R"({"value":2147483648})"};
  auto overflowed = ce::v1::data_as<narrow, C>(huge);
  expect(!overflowed.has_value()) << label;
  if (!overflowed) {
    expect(overflowed.error().code == ce::v1::errc::out_of_range) << label;
    expect(overflowed.error().where == "value") << label;
  }
  // The largest value that does fit is still accepted.
  ce::v1::event edge = minimal();
  edge.datacontenttype = "application/json";
  edge.data = ce::v1::json_text{.raw = R"({"value":2147483647})"};
  auto at_edge = ce::v1::data_as<narrow, C>(edge);
...
const boost::ut::suite<"typed-payload-layering"> payload_layering = [] {
  ...
  "the accessors are free functions, not members of event"_test = [] {
    ...
    // of ce::v1::event. Calling them as free functions is what pins that.
    ce::v1::event subject = minimal();
    const reading sent = sample();
    expect(ce::v1::set_data<reading, nlohmann_codec>(subject, sent).has_value());
    expect(ce::v1::data_as<reading, nlohmann_codec>(subject).has_value());

    static_assert(!carries_typed_payload_member<ce::v1::event>,
                  "ce::v1::event must not gain a codec-dependent member");
    expect(true);
  };
  ...
  "the extension layer stays in core"_test = [] {
    ...
    static_assert(carries_typed_extension_member<ce::v1::event>);
    expect(true);
  };
};
