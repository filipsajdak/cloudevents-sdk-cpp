...
template <class T>
[[nodiscard]] auto refused(std::string_view text, ce::v2::errc code) -> bool {
...
const boost::ut::suite<"attribute-types-refuse-invalid-text"> attribute_types_refuse = [] {
  ...
  "a required attribute refuses empty text"_test = [] {
    expect(refused<ce::v2::id>(""sv, ce::v2::errc::missing_required_attribute));
    expect(refused<ce::v2::source>(""sv, ce::v2::errc::missing_required_attribute));
    expect(refused<ce::v2::type>(""sv, ce::v2::errc::missing_required_attribute));
  };
  ...
  "a present optional attribute refuses empty text"_test = [] {
    expect(refused<ce::v2::subject>(""sv, ce::v2::errc::invalid_attribute_value));
    expect(refused<ce::v2::dataschema>(""sv, ce::v2::errc::invalid_attribute_value));
    expect(refused<ce::v2::datacontenttype>(""sv, ce::v2::errc::invalid_attribute_value));
  };

  "a value that is not encodable is refused"_test = [] {
    expect(refused<ce::v2::id>("\xC3"sv, ce::v2::errc::invalid_utf8));
    expect(refused<ce::v2::source>("\xE0\x80"sv, ce::v2::errc::invalid_utf8));
  };

  "datacontenttype must be a media type"_test = [] {
    expect(refused<ce::v2::datacontenttype>("not a media type"sv, ce::v2::errc::invalid_content_type));
    expect(accepts<ce::v2::datacontenttype>("application/json"sv));
    expect(accepts<ce::v2::datacontenttype>("text/plain; charset=utf-8"sv));
  };

  "an extension name carries both of its rules"_test = [] {
    expect(refused<ce::v2::extension_name>("Not-Lower"sv, ce::v2::errc::invalid_attribute_name));
    expect(refused<ce::v2::extension_name>(""sv, ce::v2::errc::invalid_attribute_name));
    expect(refused<ce::v2::extension_name>("id"sv, ce::v2::errc::reserved_attribute_name));
    expect(refused<ce::v2::extension_name>("data"sv, ce::v2::errc::reserved_attribute_name));
    expect(accepts<ce::v2::extension_name>("traceparent"sv));
  };
  ...
  "source is checked for non-emptiness and encoding only"_test = [] {
    expect(accepts<ce::v2::source>("/spec/test"sv));
    expect(accepts<ce::v2::source>("not a uri at all"sv));
    expect(accepts<ce::v2::source>("https://example.test/orders"sv));
  };
  ...
  "a refusal names the rule and the value"_test = [] {
    const auto built = ce::v2::extension_name::make(std::string{"Not-Lower"});
  ...
  "a borrowed value survives being copied and moved"_test = [] {
    const ce::v2::id borrowed = "A234-1234"_id;
    expect(borrowed.view() == "A234-1234"sv);

    const ce::v2::id copied{borrowed};
    expect(copied.view() == "A234-1234"sv);

    ce::v2::id to_move = "A234-1234"_id;
    const ce::v2::id moved{std::move(to_move)};
    expect(moved.view() == "A234-1234"sv);
  };

  "an owned value survives being copied and moved"_test = [] {
    auto built = ce::v2::id::make(std::string{"an identifier long enough to outgrow any small buffer"});
    expect(built.has_value());
    if (!built) {
      return;
    }

    const ce::v2::id copied{*built};
    expect(copied.view() == "an identifier long enough to outgrow any small buffer"sv);

    const ce::v2::id moved{std::move(*built)};
  ...
  "assignment works in both directions between borrowed and owned"_test = [] {
    auto owned = ce::v2::id::make(std::string{"an owned identifier long enough to allocate storage"});
    expect(owned.has_value());
    if (!owned) {
      return;
    }

    ce::v2::id subject = "borrowed"_id;
    subject = *owned;
    expect(subject.view() == "an owned identifier long enough to allocate storage"sv);

    subject = ce::v2::id{"borrowed-again"_id};
    expect(subject.view() == "borrowed-again"sv);

    ce::v2::id other = "first"_id;
  ...
  "values survive a container reallocating around them"_test = [] {
    std::vector<ce::v2::id> many;
    for (int index = 0; index < 64; ++index) {
      auto built = ce::v2::id::make("identifier-" + std::to_string(index) +
  ...
  "a map keyed by an attribute type finds by string_view"_test = [] {
    std::map<ce::v2::extension_name, int, std::less<>> extensions;
    auto name = ce::v2::extension_name::make(std::string{"traceparent"});
...
const boost::ut::suite<"attribute-literals-are-checked-when-compiled"> attribute_literals = [] {
  ...
  "a valid literal builds without a factory"_test = [] {
    const ce::v2::id identifier = "A234-1234-1234"_id;
    const ce::v2::source origin = "https://example.test/orders"_source;
    const ce::v2::type kind = "com.example.order.placed"_type;
  ...
  "the direct-initialisation spelling works too"_test = [] {
    const ce::v2::id identifier{"A234-1234-1234"};
    expect(identifier.view() == "A234-1234-1234"sv);
  };

  "every attribute has a literal"_test = [] {
    expect((ce::v2::subject{"order-99"_subject}).view() == "order-99"sv);
    expect((ce::v2::dataschema{"https://example.test/schema"_dataschema}).view() ==
           "https://example.test/schema"sv);
    expect((ce::v2::datacontenttype{"application/json"_mediatype}).view() == "application/json"sv);
    expect((ce::v2::extension_name{"traceparent"_ext}).view() == "traceparent"sv);
  };
  ...
  "specversion has one inhabitant"_test = [] {
    static_assert(std::is_empty_v<ce::v2::spec_version>);
    expect(ce::v2::spec_version{}.view() == "1.0"sv);

    const auto decoded = ce::v2::spec_version::make("0.3"sv);
    expect(!decoded.has_value());
    if (!decoded) {
      expect(decoded.error().code == ce::v2::errc::unsupported_spec_version);
    }
    expect(ce::v2::spec_version::make("1.0"sv).has_value());
  };
};
