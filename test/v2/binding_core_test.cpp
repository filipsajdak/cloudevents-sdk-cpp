...
struct exact_traits {
  ...
  [[nodiscard]] static auto encode_value(std::string_view text) -> ce::v2::result<std::string> {
    if (!ce::v2::detail::is_valid_utf8(text)) {
      return ce::v2::fail(ce::v2::errc::invalid_utf8, "attribute value is not well-formed UTF-8");
    }
...
struct no_prefix {
  ...
  static auto encode_value(std::string_view) -> ce::v2::result<std::string> { return std::string{}; }
  static auto decode_value(std::string_view) -> ce::v2::result<std::string> { return std::string{}; }
};
...
[[nodiscard]] auto read_event(const ce::v2::raw_headers& fields) -> ce::v2::result<ce::v2::event> {
  auto under_construction = binding::read_attributes<exact_traits>(fields);
  if (!under_construction) {
    return ce::v2::fail(under_construction.error().code, under_construction.error().detail,
                    under_construction.error().where);
  }
  return std::move(*under_construction).build();
}

[[nodiscard]] auto field_names(const ce::v2::raw_headers& fields) -> std::vector<std::string> {
...
const boost::ut::suite<"message-headers-from-field-list"> message_headers_field_list = [] {
  ...
  "a braced list builds the same fields as repeated add calls"_test = [] {
    const ce::v2::raw_headers listed{
        {"x_specversion", "1.0"},
        {"x_id", "1"},
        {"x_source", "/s"},
    };

    ce::v2::raw_headers added;
...
const boost::ut::suite<"binding-core-emission-order"> binding_core_emission_order = [] {
  ...
  "attributes are emitted in the order the contract fixes"_test = [] {
    const auto when = ce::v2::parse_timestamp("2026-09-21T00:00:00Z");
...
const boost::ut::suite<"binding-core-name-case"> binding_core_name_case = [] {
  ...
  "a case-sensitive binding leaves a differently-cased field alone"_test = [] {
    ce::v2::raw_headers fields{{"X_ID", "caller's own"}};
  ...
  "a case-sensitive binding does not lower an attribute name"_test = [] {
    ...
    expect(!subject);
    if (!subject) {
      expect(subject.error().code == ce::v2::errc::invalid_attribute_name);
...
const boost::ut::suite<"binding-core-round-trip"> binding_core_round_trip = [] {
  ...
  "a value the traits refuse fails the write, naming the attribute"_test = [] {
    ...
    expect(!ce::v2::subject::make(truncated).has_value());
    ...
    expect(!written);
    if (!written) {
      expect(written.error().code == ce::v2::errc::invalid_utf8);
      expect(written.error().where == "alpha"sv);
    }
  };

  "a missing required attribute names the first one absent"_test = [] {
    const ce::v2::raw_headers fields{{"x_specversion", "1.0"}, {"x_id", "1"}, {"x_source", "/s"}};

    auto subject = read_event(fields);
    expect(!subject);
    if (!subject) {
      expect(subject.error().code == ce::v2::errc::missing_required_attribute);
  ...
  "a prefixed datacontenttype is refused where the binding has a content-type field"_test = [] {
    ...
    expect(!subject);
    if (!subject) {
      expect(subject.error().code == ce::v2::errc::invalid_argument);
  ...
  "the body round-trips as bytes or as JSON text"_test = [] {
    ...
    expect(std::holds_alternative<ce::v2::binary>(binding::read_body(out.body, std::nullopt)));
    expect(std::holds_alternative<std::monostate>(
        binding::read_body(ce::v2::binary{}, subject.datacontenttype())));
  };
};
