...
struct [[=ce::v2::reflect{}]] reflected_payload {
  std::int32_t id;
  [[=ce::v2::name("score_pct")]] double score;
  std::string label;
  [[=ce::v2::skip{}]] std::int32_t internal;
};

class [[=ce::v2::reflect{}]] reflected_access {
 public:
  std::int32_t visible;
...
struct [[=ce::v2::reflect{}]] doubly_described {
  std::int32_t id;
  [[=ce::v2::name("refl_name")]] double score;
};
...
const boost::ut::suite<"described-concept"> described_concept = [] {
  ...
  "a described type satisfies the concept"_test = [] {
    static_assert(ce::v2::described<point>);
    static_assert(ce::v2::described<payload>);
    expect(true);
  };

  "an undescribed type does not"_test = [] {
    static_assert(!ce::v2::described<undescribed>);
    static_assert(!ce::v2::described<int>);
    expect(true);
  };

  "cv-qualified and reference forms agree with the bare type"_test = [] {
    static_assert(ce::v2::described<const point&>);
    static_assert(ce::v2::described<point&>);
    expect(true);
  };
};
...
const boost::ut::suite<"for-each-field-order-and-constness"> for_each_field_order = [] {
  ...
  "visits in declaration order"_test = [] {
    ...
    ce::v2::for_each_field(value, [&seen](std::string_view n, auto&) { seen.push_back(n); });
    expect(seen == std::vector<std::string_view>{"id", "score_pct", "label"});
  };

  "a non-const object yields writable members"_test = [] {
    point value{.x = 1, .y = 2};
    ce::v2::for_each_field(value, [](std::string_view n, auto& member) {
      if (n == "y") { member = 99; }
    });
    expect(value.y == 99_i);
    expect(value.x == 1_i);
  ...
  "a const object yields the same names"_test = [] {
    ...
    std::vector<std::string_view> seen;
    ce::v2::for_each_field(value, [&seen](std::string_view n, const auto&) { seen.push_back(n); });
    expect(seen == std::vector<std::string_view>{"id", "score_pct", "label"});
  };
};
...
const boost::ut::suite<"field-count-and-names-constexpr"> field_count_and_names = [] {
  ...
  "both are usable in a constant expression"_test = [] {
    static_assert(ce::v2::field_count<point> == 2);
    static_assert(ce::v2::field_count<payload> == 3);
    static_assert(ce::v2::field_count<containers> == 3);
    static_assert(ce::v2::field_names<point>().size() == 2);
    static_assert(ce::v2::field_names<point>()[0] == "x");
    expect(true);
  };
};
...
const boost::ut::suite<"macro-backend-describe"> macro_backend_describe = [] {
  ...
  "CE_DESCRIBE names every listed member"_test = [] {
    static_assert(ce::v2::backend_of<point> == ce::v2::describe_backend::macro);
  ...
  "member pointers address the right members"_test = [] {
    point value{.x = 4, .y = 7};
    std::int32_t sum = 0;
    ce::v2::for_each_field(value, [&sum](std::string_view, auto& member) { sum += member; });
    expect(sum == 11_i);
  };
};
...
const boost::ut::suite<"macro-backend-rename"> macro_backend_rename = [] {
  ...
  "CE_FIELD replaces the wire name, leaving the others alone"_test = [] {
    static_assert(ce::v2::field_names<payload>()[0] == "id");
    static_assert(ce::v2::field_names<payload>()[1] == "score_pct");
    static_assert(ce::v2::field_names<payload>()[2] == "label");
    expect(true);
  };
};
...
const boost::ut::suite<"reflection-backend-describe"> reflection_backend_describe = [] {
  ...
  "reflection describes an opted-in type"_test = [] {
    static_assert(ce::v2::backend_of<reflected_payload> == ce::v2::describe_backend::reflection);
  ...
  "an annotation renames, and skip excludes"_test = [] {
    static_assert(ce::v2::field_count<reflected_payload> == 3);
    static_assert(ce::v2::field_names<reflected_payload>()[1] == "score_pct");
    expect(true);
  };

  "reflection does not adopt a type that did not opt in"_test = [] {
    static_assert(!ce::v2::described<not_opted_in>);
    expect(true);
  };
#else
...
const boost::ut::suite<"reflection-backend-public-only"> reflection_backend_public_only = [] {
  ...
  "private members are excluded"_test = [] {
    static_assert(ce::v2::field_count<reflected_access> == 1);
    static_assert(ce::v2::field_names<reflected_access>()[0] == "visible");
    expect(true);
  };
#else
  "not exercised without -freflection"_test = [] {
...
const boost::ut::suite<"macro-precedence-under-reflection"> macro_precedence = [] {
  ...
  "a macro description wins over reflection"_test = [] {
    static_assert(ce::v2::backend_of<doubly_described> == ce::v2::describe_backend::macro);
    static_assert(ce::v2::field_names<doubly_described>()[1] == "macro_name");
    expect(true);
  };
#else
  "the macro backend is the only one here"_test = [] {
    static_assert(ce::v2::backend_of<payload> == ce::v2::describe_backend::macro);
    expect(true);
  };
#endif
};
...
const boost::ut::suite<"supported-member-types"> supported_member_types = [] {
  ...
  "scalars, strings and the supported containers are accepted"_test = [] {
    static_assert(ce::v2::members_supported<point>());
    static_assert(ce::v2::members_supported<payload>());
    static_assert(ce::v2::members_supported<containers>());
    expect(true);
  };
};
...
const boost::ut::suite<"describe-backend-parity"> describe_backend_parity = [] {
  ...
  "the reflection backend produces the same description"_test = [] {
    ...
    const auto macro_names = ce::v2::field_names<payload>();
    const auto reflected_names = ce::v2::field_names<reflected_payload>();
