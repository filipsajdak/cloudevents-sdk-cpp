...
/// can substitute for.
[[nodiscard]] auto read_http_wire(std::string_view sdk, std::string_view name) -> ce::v2::message {
  ...
  if (!encoded.empty()) {
    if (auto decoded = ce::v2::base64_decode(encoded)) {
      out.body = *decoded;
    }
  }
  return out;
...
[[nodiscard]] auto payload_bytes(const ce::v2::event& subject) -> std::vector<std::byte> {
  ...
  if (const auto* bytes = std::get_if<ce::v2::binary>(&subject.data())) {
    return *bytes;
  }
  if (const auto* json = std::get_if<ce::v2::json_text>(&subject.data())) {
    std::vector<std::byte> out;
...
void check_cross_sdk_agreement(std::string_view label) {
  ...
  for (const auto name : documents) {
    std::vector<ce::v2::event> decoded;
    for (const auto sdk : producers) {
...
void check_binary_payload_agreement(std::string_view label) {
  ...
  const ce::v2::binary expected{std::byte{0x00}, std::byte{0x01}, std::byte{0x02}, std::byte{0xFF}};
  for (const auto sdk : producers) {
    ...
    expect(std::holds_alternative<ce::v2::binary>(decoded->data())) << label << ": " << sdk;
    if (const auto* bytes = std::get_if<ce::v2::binary>(&decoded->data())) {
...
void check_extension_agreement(std::string_view label) {
  ...
  for (const auto sdk : producers) {
    ...
    auto tracing = decoded->template get<ce::v2::ext::tracing>();
    ...
    auto partitioning = decoded->template get<ce::v2::ext::partitioning>();
    ...
    auto rate = decoded->template get<ce::v2::ext::sampled_rate>();
...
const boost::ut::suite<"interop-golden-corpus"> interop = [] {
  ...
  "every producer's batch decodes here"_test = [] {
    ...
    for (const auto sdk : batch_producers) {
      ...
      if (events) {
        ...
        // survive here.
        expect(format::encode_batch(std::span<const ce::v2::event>{*events}).has_value()) << sdk;
      }
    }
  };
  ...
  "the SDKs disagree about the offset in a timestamp"_test = [] {
    ...
    // But ce::v2::timestamp compares its TEXT, because SWR-CORE-0009 requires a
  ...
  "the SDKs disagree about how to carry a non-JSON payload"_test = [] {
    ...
    expect(std::holds_alternative<ce::v2::binary>(from_java->data())) << "Java changed representation";
...
const boost::ut::suite<"interop-http-binary-mode"> interop_http_binary_mode = [] {
  ...
  "go binary-mode headers decode to the documented event"_test = [] {
    for (const auto name : http_wire_names) {
      const ce::v2::message wire = read_http_wire("go", name);
      expect(!wire.header_fields.empty()) << name;

      auto from_headers =
          ce::v2::http::from_message<nlohmann_codec, ce::v2::http::literal_values>(wire);
      expect(bool{from_headers}) << name << " headers did not decode";
      if (!from_headers) {
        continue;
      }
      auto from_document = ce::v2::json_format<nlohmann_codec>::decode(read_golden("go", name));
  ...
  "a literal percent from go needs the opt-in policy"_test = [] {
    const ce::v2::message wire = read_http_wire("go", "percent_in_subject");
    expect(!wire.header_fields.empty());

    auto conformant = ce::v2::http::from_message<nlohmann_codec>(wire);
    expect(!conformant) << "the conformant reader accepted an unescaped percent";
    if (!conformant) {
      expect(conformant.error().code == ce::v2::errc::parse_error);
    }

    auto compatible = ce::v2::http::from_message<nlohmann_codec, ce::v2::http::literal_values>(wire);
    ...
    if (compatible) {
      ...
      if (pct != nullptr) {
        ...
        expect(bool{*pct == ce::v2::attribute_value{std::string{"a%20b"}}});
      }
    }
  };
};
...
const boost::ut::suite<"interop-behaviour-audit"> interop_behaviour_audit = [] {
  ...
  "a non-UTC offset survives a write here, unlike Go"_test = [] {
    ...
    const auto when = ce::v2::parse_timestamp("2026-09-20T12:34:56.123456789+02:00");
    expect(when.has_value());
    if (!when) {
      return;
    }
    const ce::v2::event subject{"1"_id, "/probe"_source, "com.example.probe"_type, {.time = *when}};

    auto written = ce::v2::http::to_message<nlohmann_codec>(subject, ce::v2::content_mode::binary_mode);
    expect(bool{written});
    if (written) {
  ...
  "a lowercase RFC 3339 time is accepted here, unlike Go"_test = [] {
    ...
    const ce::v2::message request{.header_fields = {{"ce-specversion", "1.0"},
                                                {"ce-id", "1"},
                                                {"ce-source", "/s"},
                                                {"ce-type", "t"},
                                                {"ce-time", "2026-09-20t12:34:56z"}}};

    auto decoded = ce::v2::http::from_message<nlohmann_codec>(request);
    expect(bool{decoded});
    if (decoded) {
  ...
  "an empty required or optional attribute is refused here, unlike Go"_test = [] {
    ...
    const auto with = [](std::string_view name, std::string_view value) {
      ce::v2::message request{.header_fields = {{"ce-specversion", "1.0"},
                                            {"ce-id", "1"},
                                            {"ce-source", "/s"},
                                            {"ce-type", "t"}}};
    ...
    auto empty_subject = ce::v2::http::from_message<nlohmann_codec>(with("ce-subject", ""));
    expect(!empty_subject);
    if (!empty_subject) {
      expect(empty_subject.error().code == ce::v2::errc::invalid_attribute_value);
    }

    auto empty_type = ce::v2::http::from_message<nlohmann_codec>(with("ce-type", ""));
    expect(!empty_type);
    ...
    const auto empty = ce::v2::subject::make(""sv);
    expect(!empty);
    if (!empty) {
      expect(empty.error().code == ce::v2::errc::invalid_attribute_value);
    }
  };
