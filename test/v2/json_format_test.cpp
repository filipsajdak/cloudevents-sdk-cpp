...
template <class C>
[[nodiscard]] auto encoded_document(const ce::v2::event& subject) -> ce::v2::result<typename C::value> {
  auto text = ce::v2::json_format<C>::encode(subject);
  if (!text) {
    return ce::v2::fail(text.error().code, text.error().detail, text.error().where);
  }
  return C::parse(*text);
}
...
void check_entry_points(std::string_view label) {
  ...
  if (back->time()) {
    expect(ce::v2::to_string(*back->time()) == "2018-04-05T17:31:00Z") << label;
  }
  expect(back->extensions().size() == 3U) << label;

  const std::vector<ce::v2::event> many{subject, base_event()};
  auto batch_text = format::encode_batch(std::span<const ce::v2::event>{many});
  ...
  expect(!ce::v2::id::make(""sv).has_value()) << label;
}
...
void check_integer_attribute(std::string_view label) {
  ...
  const ce::v2::event subject = base_event({.extensions = {{"seq"_ext, std::int32_t{7}},
                                                       {"low"_ext, lowest},
                                                       {"high"_ext, highest}}});
  ...
  for (const auto& [name, value] : expected) {
    ...
    expect(C::kind_of(*member) == ce::v2::json::kind::integer) << label << ": " << name;
  ...
  auto back = ce::v2::json_format<C>::decode(*text);
...
void check_integer_rejections(std::string_view label) {
  ...
  const std::pair<std::string_view, ce::v2::errc> cases[] = {
      // Above int32 max, below int32 min: truncating either would hand the
      // caller an event the peer never sent.
      {R"("big":9999999999})"sv, ce::v2::errc::out_of_range},
      {R"("small":-9999999999})"sv, ce::v2::errc::out_of_range},
      {R"("edge":2147483648})"sv, ce::v2::errc::out_of_range},
      {R"("edge":-2147483649})"sv, ce::v2::errc::out_of_range},
      // A fractional number is not an Integer at all.
      {R"("ratio":1.5})"sv, ce::v2::errc::type_mismatch},
      {R"("ratio":-0.25})"sv, ce::v2::errc::type_mismatch},
  };
  ...
  for (const auto& [tail, code] : cases) {
    ...
    if (!decoded) {
      expect(decoded.error().code == code) << label << ": " << document << " gave "
                                           << ce::v2::to_string_view(decoded.error().code);
    }
  }
...
void check_binary_attribute(std::string_view label) {
  ...
  const ce::v2::event subject = base_event({.extensions = {{"blob"_ext, octets}}});
  ...
  expect(C::kind_of(*member) == ce::v2::json::kind::string) << label;
  const auto held = C::as_string(*member);
  expect(held.has_value()) << label;
  if (!held) {
    return;
  }
  expect(*held == ce::v2::base64_encode(octets)) << label << ": " << *held;
  ...
  const auto recovered = ce::v2::base64_decode(*held);
...
void check_textual_attributes(std::string_view label) {
  ...
  const ce::v2::event subject = base_event({
      .dataschema = "https://example.com/schema"_dataschema,
      .time = *parsed_time,
      .extensions = {{"schemaurl"_ext, ce::v2::uri{"https://example.com/x"}},
                     {"relref"_ext, ce::v2::uri_ref{"/relative/path"}},
  ...
  for (const auto& [name, rendered] : expected) {
    ...
    expect(C::kind_of(*member) == ce::v2::json::kind::string) << label << ": " << name;
    const auto held = C::as_string(*member);
    expect(held.has_value()) << label << ": " << name;
    if (held) {
...
void check_json_text_data(std::string_view label) {
  ...
  const ce::v2::event subject =
      base_event({.datacontenttype = "application/json"_mediatype,
                  .data = ce::v2::json_text{.raw = R"({"k":1,"list":[1,2]})"}});
  ...
  expect(C::kind_of(*data) == ce::v2::json::kind::object) << label;
  ...
  if (list != nullptr) {
    expect(C::kind_of(*list) == ce::v2::json::kind::array) << label;
  ...
  auto refused = ce::v2::json_format<C>::encode(broken);
...
void check_binary_data(std::string_view label) {
  ...
  const ce::v2::binary octets{std::byte{1}, std::byte{2}, std::byte{0xFF}};
  const ce::v2::event subject = base_event({.data = octets});
  ...
  expect(C::kind_of(*encoded) == ce::v2::json::kind::string) << label;
  ...
  auto empty_document = encoded_document<C>(base_event({.data = ce::v2::binary{}}));
...
void check_string_data(std::string_view label) {
  ...
  expect(C::kind_of(*data) == ce::v2::json::kind::string) << label;
...
void check_decode_data_base64(std::string_view label) {
  ...
  if (std::holds_alternative<ce::v2::binary>(decoded->data())) {
    expect(std::get<ce::v2::binary>(decoded->data()) == octets) << label;
  }

  // Unpadded base64 from a peer decodes to the same octets.
  auto unpadded_source = ce::v2::base64_encode(ce::v2::binary{std::byte{0xAB}});
  ...
  if (from_unpadded && std::holds_alternative<ce::v2::binary>(from_unpadded->data())) {
    expect(std::get<ce::v2::binary>(from_unpadded->data()) == ce::v2::binary{std::byte{0xAB}}) << label;
  }
  ...
  if (!bad) {
    expect(bad.error().code == ce::v2::errc::invalid_base64) << label;
  }
...
void check_decode_data_as_json_text(std::string_view label) {
  ...
  expect(decoded.has_value()) << label;
  if (decoded) {
    expect(std::holds_alternative<ce::v2::json_text>(decoded->data())) << label;
    if (std::holds_alternative<ce::v2::json_text>(decoded->data())) {
      // The raw text must itself be well-formed JSON carrying the same value.
      const auto& raw = std::get<ce::v2::json_text>(decoded->data()).raw;
  ...
  for (const auto& [content_type, payload] : json_typed) {
    ...
    if (typed) {
      expect(std::holds_alternative<ce::v2::json_text>(typed->data())) << label << ": " << document;
    }
  }
  ...
  if (numeric) {
    expect(std::holds_alternative<ce::v2::json_text>(numeric->data())) << label;
  }
}
...
void check_data_conflict(std::string_view label) {
  ...
  for (const auto document : {
    ...
    expect(!decoded.has_value()) << label << ": should reject " << document;
    if (!decoded) {
      expect(decoded.error().code == ce::v2::errc::data_conflict) << label << ": " << document;
    }
  }
...
void check_extension_type_mapping(std::string_view label) {
  ...
  const ce::v2::event subject =
      base_event({.extensions = {{"schemaurl"_ext, ce::v2::uri{"https://example.com/x"}}}});
  auto text = format::encode(subject);
  expect(text.has_value()) << label;
  if (text) {
    auto back = format::decode(*text);
  ...
  for (const auto document : {
    ...
    if (!rejected) {
      expect(rejected.error().code == ce::v2::errc::type_mismatch) << label << ": " << document;
    }
  }
}
...
void check_floating_extension_rejected(std::string_view label) {
  ...
  for (const auto document : {
    ...
    expect(!decoded.has_value()) << label << ": should reject " << document;
    if (!decoded) {
      expect(decoded.error().code == ce::v2::errc::type_mismatch)
          << label << ": " << document << " gave " << ce::v2::to_string_view(decoded.error().code);
...
void check_empty_batch(std::string_view label) {
  ...
  auto text = format::encode_batch(std::span<const ce::v2::event>{none});
  expect(text.has_value()) << label;
  if (!text) {
    return;
  }
...
void check_spec_examples(std::string_view label) {
  ...
  if (decoded->time()) {
    expect(ce::v2::to_string(*decoded->time()) == "2018-04-05T17:31:00Z") << label;
  }
  ...
  if (again) {
    // Wrapped in bool: ut would otherwise try to print a ce::v2::event on failure.
  ...
  auto batch_re_encoded = format::encode_batch(std::span<const ce::v2::event>{*batch_decoded});
  ...
  if (!rejected_batch) {
    expect(rejected_batch.error().code == ce::v2::errc::missing_required_attribute) << label;
  }
}
...
const boost::ut::suite<"json-format-entry-points"> format_entry_points = [] {
  ...
  "the four operations have the declared signatures"_test = [] {
    using format = ce::v2::json_format<nlohmann_codec>;
    static_assert(std::is_same_v<decltype(format::encode(std::declval<const ce::v2::event&>())),
                                 ce::v2::result<std::string>>);
    static_assert(std::is_same_v<decltype(format::decode(""sv)), ce::v2::result<ce::v2::event>>);
    static_assert(
        std::is_same_v<decltype(format::encode_batch(std::declval<std::span<const ce::v2::event>>())),
                       ce::v2::result<std::string>>);
    static_assert(
        std::is_same_v<decltype(format::decode_batch(""sv)), ce::v2::result<std::vector<ce::v2::event>>>);
    static_assert(std::is_same_v<decltype(ce::v2::json_format<mini_codec>::decode(""sv)),
  ...
  "an unsupported version is reported before any other rule"_test = [] {
    const auto decoded = ce::v2::json_format<nlohmann_codec>::decode(
        R"({"specversion":"0.3","id":"","source":"","type":""})"sv);
    expect(!decoded.has_value());
    expect(!decoded.has_value() && decoded.error().code == ce::v2::errc::unsupported_spec_version);
  };
  ...
  ce_test::for_each_codec([]<class C>(std::string_view codec) {
    test(std::string{codec} + ": a context attribute is reported before the rest") = [codec] {
      const auto decoded = ce::v2::json_format<C>::decode(
          R"({"Bad":1,"data":1,"data_base64":"AA==","specversion":"1.0","source":"/s","type":"t"})"sv);
      expect(!decoded.has_value()) << codec;
      if (!decoded) {
        expect(decoded.error().code == ce::v2::errc::missing_required_attribute) << codec;
    ...
    test(std::string{codec} + ": the payload is reported before the extensions") = [codec] {
      ...
      expect(!decoded.has_value()) << codec;
      if (!decoded) {
        expect(decoded.error().code == ce::v2::errc::data_conflict) << codec;
      }
    };
  });
  ...
  "a repeated context attribute is read from its first occurrence"_test = [] {
    const auto decoded = ce::v2::json_format<mini_codec>::decode(
...
void check_extension_name_grammar(std::string_view label) {
  ...
  for (const auto document : {
    ...
    expect(!decoded.has_value()) << label << ": should reject " << document;
    if (!decoded) {
      expect(decoded.error().code == ce::v2::errc::invalid_attribute_name) << label << ": " << document;
    }
  }
...
const boost::ut::suite<"json-format-media-types"> media_types = [] {
  ...
  "the two media types are fixed and codec-independent"_test = [] {
    static_assert(ce::v2::json_format<nlohmann_codec>::content_type ==
                  "application/cloudevents+json"sv);
    static_assert(ce::v2::json_format<nlohmann_codec>::batch_content_type ==
                  "application/cloudevents-batch+json"sv);
    static_assert(ce::v2::json_format<mini_codec>::content_type == "application/cloudevents+json"sv);
    static_assert(ce::v2::json_format<mini_codec>::batch_content_type ==
                  "application/cloudevents-batch+json"sv);
    // The format re-exports what the JSON layer declares, so there is one
    // definition rather than two that can drift.
    static_assert(ce::v2::json_format<mini_codec>::content_type == ce::v2::json::content_type);
    static_assert(ce::v2::json_format<mini_codec>::batch_content_type == ce::v2::json::batch_content_type);
    expect(true);
  };

  "both are JSON media types by the SDK's own test"_test = [] {
    expect(ce::v2::is_json_content_type(ce::v2::json_format<nlohmann_codec>::content_type));
    expect(ce::v2::is_json_content_type(ce::v2::json_format<mini_codec>::batch_content_type));
  };
};
...
void check_out_of_range_integer_extension(std::string_view label) {
  ...
  for (const auto document : {
    ...
    expect(code == ce::v2::errc::out_of_range || code == ce::v2::errc::parse_error)
        << label << ": " << document << " gave " << ce::v2::to_string_view(code);
  }
...
const boost::ut::suite<"format-rules-shared-with-the-bench"> format_rules_shared = [] {
  ...
  ce_test::for_each_codec([]<class C>(std::string_view codec) {
    test(std::string{codec}) = [codec] {
      ce::v2::checks::check_format_rules<C>(
