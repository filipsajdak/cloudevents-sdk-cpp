...
/// a case is about.
[[nodiscard]] auto record_with(ce::v2::raw_headers::entry extra) -> ce::v2::message {
  return ce::v2::message{.header_fields = {{"ce_specversion", "1.0"},
                                       {"ce_id", "1"},
                                       {"ce_source", "/s"},
                                       {"ce_type", "t"},
                                       std::move(extra)}};
...
void check_binary_mode(std::string_view codec) {
  ...
  const ce::v2::event subject = base_event({.subject = "s"_subject});

  auto laid_out = ce::v2::kafka::to_message<Codec>(subject, ce::v2::content_mode::binary_mode);
  ...
  auto read_back = ce::v2::kafka::from_message<Codec>(*laid_out);
...
void check_structured_mode(std::string_view codec) {
  ...
  auto laid_out = ce::v2::kafka::to_message<Codec>(subject, ce::v2::content_mode::structured);
  expect(bool{laid_out}) << codec;
  if (!laid_out) {
    return;
  }
...
void check_values_are_unescaped_utf8(std::string_view codec) {
  ...
  const ce::v2::event subject = base_event({.subject = "a b\xC3\xBC"_subject});
...
void check_invalid_utf8_is_refused(std::string_view codec) {
  ...
  expect(!ce::v2::subject::make(truncated)) << codec;
  const ce::v2::event subject = base_event({.extensions = {{"alpha"_ext, truncated}}});

  auto laid_out = ce::v2::kafka::to_message<Codec>(subject, ce::v2::content_mode::binary_mode);
  expect(!laid_out) << codec;
  if (!laid_out) {
    expect(laid_out.error().code == ce::v2::errc::invalid_utf8) << codec;
...
void check_keys_are_byte_exact(std::string_view codec) {
  ...
  ce::v2::message carrier{.header_fields = {{"CE_ID", "the caller's own"}}};
  const ce::v2::event subject = base_event();
  auto written = ce::v2::binding::write_attributes<ce::v2::kafka::detail::kafka_traits>(
  ...
  auto read_back = ce::v2::kafka::from_message<Codec>(record_with({"CE_SOURCE", "/not-the-source"}));
  expect(bool{read_back}) << codec;
  if (read_back) {
...
void check_batch_refused(std::string_view codec) {
  ...
  const ce::v2::event subject = base_event();
  auto laid_out = ce::v2::kafka::to_message<Codec>(subject, ce::v2::content_mode::batched);
  expect(!laid_out) << codec;
  if (!laid_out) {
    expect(laid_out.error().code == ce::v2::errc::invalid_argument) << codec;
  }
  ...
  const ce::v2::message incoming{
      .header_fields = {{"content-type", "application/cloudevents-batch+json"}},
      .body = ce::v2::to_bytes("[]"),
  };
  auto read_back = ce::v2::kafka::from_message<Codec>(incoming);
  expect(!read_back) << codec;
  if (!read_back) {
    expect(read_back.error().code == ce::v2::errc::invalid_argument) << codec;
  }
}
...
void check_not_a_cloudevent(std::string_view codec) {
  ...
  const ce::v2::message incoming{
      ...
      .body = ce::v2::to_bytes("{\"unrelated\":true}"),
  };

  auto read_back = ce::v2::kafka::from_message<Codec>(incoming);
  expect(!read_back) << codec;
  if (!read_back) {
    expect(read_back.error().code == ce::v2::errc::not_a_cloudevent) << codec;
  }
}
...
void check_record_and_key_mapper(std::string_view codec) {
  ...
  ce::v2::event subject = base_event();
  expect(bool{subject.set(ce::v2::ext::partitioning{.partitionkey = "customer-42"})}) << codec;
  const ce::v2::event before = subject;

  // Not naming a mapper is the specification's default behaviour.
  auto plain = ce::v2::kafka::to_record<Codec>(subject, ce::v2::content_mode::binary_mode);
  expect(bool{plain}) << codec;
  if (plain) {
    expect(!plain->key.has_value()) << codec;
  }

  auto keyed = ce::v2::kafka::to_record<Codec, ce::v2::kafka::partitionkey_mapper>(
  ...
  auto without = ce::v2::kafka::to_record<Codec, ce::v2::kafka::partitionkey_mapper>(
      base_event(), ce::v2::content_mode::binary_mode);
...
const boost::ut::suite<"kafka-record-and-key-mapper"> kafka_record_and_key_mapper = [] {
  ...
  static_assert(ce::v2::kafka::key_mapper<ce::v2::kafka::no_key_mapper>);
  static_assert(ce::v2::kafka::key_mapper<ce::v2::kafka::partitionkey_mapper>);
  static_assert(std::is_aggregate_v<ce::v2::kafka::record>);
