...
struct mini_codec {
  ...
  [[nodiscard]] static auto make_bool(bool boolean) -> value {
    return value{.tag = ce::v2::json::kind::boolean, .boolean = boolean};
  }
  [[nodiscard]] static auto make_int(std::int64_t integer) -> value {
    return value{.tag = ce::v2::json::kind::integer, .integer = integer};
  }
  [[nodiscard]] static auto make_double(double number) -> value {
    return value{.tag = ce::v2::json::kind::floating, .number = number};
  }
  [[nodiscard]] static auto make_string(std::string_view text) -> value {
    return value{.tag = ce::v2::json::kind::string, .text = std::string{text}};
  }
  [[nodiscard]] static auto make_array() -> value {
    return value{.tag = ce::v2::json::kind::array};
  }
  [[nodiscard]] static auto make_object() -> value {
    return value{.tag = ce::v2::json::kind::object};
  }
  ...
  // --- inspection ----------------------------------------------------------
  [[nodiscard]] static auto kind_of(const value& subject) -> ce::v2::json::kind { return subject.tag; }
  ...
  [[nodiscard]] static auto size_of(const value& subject) -> std::size_t {
    return subject.tag == ce::v2::json::kind::object ? subject.members.size() : subject.elements.size();
  }

  // --- extraction ----------------------------------------------------------
  [[nodiscard]] static auto as_bool(const value& subject) -> ce::v2::result<bool> {
    if (subject.tag != ce::v2::json::kind::boolean) {
      return ce::v2::fail(ce::v2::errc::type_mismatch, "not a JSON boolean");
    }
    return subject.boolean;
  }

  [[nodiscard]] static auto as_int(const value& subject) -> ce::v2::result<std::int64_t> {
    // A double never satisfies this, even when it holds an integral value.
    if (subject.tag != ce::v2::json::kind::integer) {
      return ce::v2::fail(ce::v2::errc::type_mismatch, "not a JSON integer");
    }
    return subject.integer;
  }

  [[nodiscard]] static auto as_double(const value& subject) -> ce::v2::result<double> {
    if (subject.tag == ce::v2::json::kind::floating) {
      return subject.number;
    }
    if (subject.tag == ce::v2::json::kind::integer) {
      return static_cast<double>(subject.integer);
    }
    return ce::v2::fail(ce::v2::errc::type_mismatch, "not a JSON number");
  }

  [[nodiscard]] static auto as_string(const value& subject) -> ce::v2::result<std::string_view> {
    if (subject.tag != ce::v2::json::kind::string) {
      return ce::v2::fail(ce::v2::errc::type_mismatch, "not a JSON string");
    }
  ...
  // --- text <-> DOM --------------------------------------------------------
  [[nodiscard]] static auto parse(std::string_view text) -> ce::v2::result<value>;
...
auto parse_value(reader& input) -> ce::v2::result<mini_codec::value>;
...
inline auto parse_string_body(reader& input) -> ce::v2::result<std::string> {
  if (input.peek() != '"') {
    return ce::v2::fail(ce::v2::errc::parse_error, "expected a string");
  }
  ++input.pos;
  std::string out;
  while (!input.done()) {
      ...
      switch (escape) {
        ...
        case 'u': {
          if (input.pos + 4 > input.text.size()) {
            return ce::v2::fail(ce::v2::errc::parse_error, "truncated \\u escape");
          }
          std::uint32_t code = 0;
          for (int i = 0; i < 4; ++i) {
            ...
            if (digit >= '0' && digit <= '9') {
              ...
              return ce::v2::fail(ce::v2::errc::parse_error, "bad hex in \\u escape");
            }
          }
          ...
          if (code >= 0xD800 && code <= 0xDBFF) {
            if (input.pos + 6 > input.text.size() || input.text[input.pos] != '\\' ||
              ...
              return ce::v2::fail(ce::v2::errc::parse_error, "high surrogate with no low surrogate");
            }
            input.pos += 2;
            std::uint32_t low = 0;
            for (int i = 0; i < 4; ++i) {
            ...
            if (low < 0xDC00 || low > 0xDFFF) {
              return ce::v2::fail(ce::v2::errc::parse_error, "high surrogate not followed by a low one");
            }
            code = 0x10000U + ((code - 0xD800U) << 10U) + (low - 0xDC00U);
          } else if (code >= 0xDC00 && code <= 0xDFFF) {
            return ce::v2::fail(ce::v2::errc::parse_error, "low surrogate with no high surrogate");
          }
          append_utf8(code, out);
          break;
        }
        default: return ce::v2::fail(ce::v2::errc::parse_error, "unknown escape");
      }
      continue;
    }
    out.push_back(character);
  }
  return ce::v2::fail(ce::v2::errc::parse_error, "unterminated string");
}
...
inline auto parse_number(reader& input) -> ce::v2::result<mini_codec::value> {
  ...
  if (token.empty() || token == "-") {
    return ce::v2::fail(ce::v2::errc::parse_error, "expected a number");
  }
  ...
  if (fractional) {
    ...
    if (end != token.c_str() + token.size() || errno == ERANGE) {
      return ce::v2::fail(ce::v2::errc::parse_error, "not a JSON number", token);
    }
  ...
  if (code != std::errc{} || stop != last) {
    return ce::v2::fail(ce::v2::errc::parse_error, "not a JSON integer", token);
  }
  return mini_codec::make_int(parsed);
}

inline auto parse_value(reader& input) -> ce::v2::result<mini_codec::value> {
  if (input.depth >= max_parse_depth) {
    return ce::v2::fail(ce::v2::errc::parse_error, "JSON nested beyond the depth limit");
  }
  const depth_guard guard{input};
  input.skip_space();
  if (input.done()) {
    return ce::v2::fail(ce::v2::errc::parse_error, "unexpected end of input");
  }
  const char character = input.peek();
  ...
  if (character == '{') {
    ...
    while (true) {
      ...
      if (!key) {
        return ce::v2::fail(key.error().code, key.error().detail);
      }
      input.skip_space();
      if (input.peek() != ':') {
        return ce::v2::fail(ce::v2::errc::parse_error, "expected ':'");
      }
      ++input.pos;
      ...
      return ce::v2::fail(ce::v2::errc::parse_error, "expected ',' or '}'");
    }
  }
  ...
  if (character == '"') {
    ...
    if (!text) {
      return ce::v2::fail(text.error().code, text.error().detail);
    }
...
inline void dump_value(std::string& out, const mini_codec::value& subject) {
  switch (subject.tag) {
    case ce::v2::json::kind::null: out += "null"; break;
    case ce::v2::json::kind::boolean: out += subject.boolean ? "true" : "false"; break;
    case ce::v2::json::kind::integer: out += std::to_string(subject.integer); break;
    case ce::v2::json::kind::floating: {
    ...
    case ce::v2::json::kind::string: dump_string(out, subject.text); break;
    case ce::v2::json::kind::array: {
      out.push_back('[');
      bool first = true;
...
inline auto mini_codec::parse(std::string_view text) -> ce::v2::result<value> {
  ...
  input.skip_space();
  if (!input.done()) {
    return ce::v2::fail(ce::v2::errc::parse_error, "trailing characters after the document");
  }
  return parsed;
}
...
static_assert(ce::v2::json::json_codec<mini_codec>);
