...
auto probe() -> report {
  using format = ce::v2::json_format<ce::test::mini_codec>;
  using namespace ce::v2::literals;
  static_assert(ce::v2::json::json_codec<ce::test::mini_codec>);

  const ce::v2::event subject{"1"_id, "/spec/test"_source, "com.example.thing"_type,
  ...
  const bool round_tripped = decoded.has_value() && decoded->id() == subject.id() &&
                             ...
                             std::holds_alternative<ce::v2::json_text>(decoded->data());

  // The typed payload accessors, over the same user-supplied codec.
  ce::v2::event typed{"2"_id, "/spec/test"_source, "com.example.parcel"_type};
  ...
  ce::v2::set_data<parcel, ce::test::mini_codec>(typed, sent);
  auto read = ce::v2::data_as<parcel, ce::test::mini_codec>(typed);
  ...
  ce::v2::event tagged{"3"_id, "/spec/test"_source, "com.example.traced"_type};
  const bool wrote_extension =
      tagged.set(ce::v2::ext::tracing{.traceparent = "00-a-b-01", .tracestate = {}}).has_value();
  auto tracing = tagged.get<ce::v2::ext::tracing>();
