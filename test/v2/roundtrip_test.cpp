    ...
    for (const auto& source : sources) {
      for (const auto& type : types) {
        for (int shape = 0; shape < 4; ++shape) {
          ...
          ce::v2::event::builder next{
              .id = attribute<ce::v2::id>(id),
              .source = attribute<ce::v2::source>(source),
              .type = attribute<ce::v2::type>(type),
          };
          auto& rest = next.rest;

          if (index % 2 == 0) {
            rest.subject = attribute<ce::v2::subject>("subject-" + std::to_string(index));
          }
          if (index % 3 == 0) {
            rest.dataschema =
                attribute<ce::v2::dataschema>("https://example.test/schema/" + std::to_string(index));
          }
          if (index % 5 == 0) {
            if (auto parsed = ce::v2::parse_timestamp("2026-09-20T12:34:56.789-05:30")) {
              rest.time = *parsed;
            }
          }
          if (index % 11 == 0) {
            rest.extensions.insert_or_assign("seq"_ext, ce::v2::attribute_value{std::int32_t{42}});
            rest.extensions.insert_or_assign("ok"_ext, ce::v2::attribute_value{true});
          }
          ...
          switch (shape) {
            ...
            case 1:
              ...
              rest.data = ce::v2::json_text{.raw = R"({"n":)" + std::to_string(index) + "}"};
              break;
            case 2:
            ...
            default:
              ...
              rest.data = ce::v2::binary{std::byte{0x00}, std::byte{0x7F},
          ...
          if (this_index % 7 == 0) {
            boost::ut::expect(bool{
                built->set(ce::v2::ext::tracing{.traceparent = "00-a-b-01", .tracestate = {}})});
          }
...
void check_structured_message_roundtrip(std::string_view label) {
  ...
  for (const auto& [subject, index] : generate()) {
    auto message = ce::v2::http::to_message<C>(subject, ce::v2::content_mode::structured);
    expect(message.has_value()) << label << " #" << index;
    if (!message) {
      continue;
    }
...
void check_batched_roundtrip(std::string_view label) {
  ...
  auto message = ce::v2::http::to_batch_message<C>(std::span<const ce::v2::event>{events});
  expect(message.has_value()) << label;
  if (!message) {
    return;
  }
  auto from_message = ce::v2::http::from_batch_message<C>(*message);
...
const boost::ut::suite<"roundtrip-property"> roundtrip_property = [] {
  ...
  "the generator varies what it claims to vary"_test = [] {
    ...
    for (const auto& [subject, index] : events) {
      ...
      with_json += std::holds_alternative<ce::v2::json_text>(subject.data()) ? 1 : 0;
      with_text += std::holds_alternative<std::string>(subject.data()) ? 1 : 0;
      with_binary += std::holds_alternative<ce::v2::binary>(subject.data()) ? 1 : 0;
