...
[[nodiscard]] constexpr auto check_non_empty(std::string_view text)
    -> std::optional<ce::v2::static_error> {
  if (text.empty()) {
    return ce::v2::static_error{
        .code = ce::v2::errc::missing_required_attribute,
...
const boost::ut::suite<"static-error-survives-constant-evaluation"> static_error_constexpr = [] {
  ...
  "a diagnosis is usable in a constant expression"_test = [] {
    ...
    static_assert(absent->code == ce::v2::errc::missing_required_attribute);
  ...
  "a long detail is no different from a short one"_test = [] {
    static constexpr ce::v2::static_error verbose{
        .code = ce::v2::errc::invalid_attribute_value,
    ...
    static_assert(verbose.code == ce::v2::errc::invalid_attribute_value);
    static_assert(verbose.detail.size() > 64U);
    expect(true);
  };

  "it is a literal type and compares by value"_test = [] {
    static_assert(std::is_trivially_copyable_v<ce::v2::static_error>);
    static_assert(ce::v2::static_error{.code = ce::v2::errc::parse_error} ==
                  ce::v2::static_error{.code = ce::v2::errc::parse_error});
    static_assert(!(ce::v2::static_error{.code = ce::v2::errc::parse_error} ==
                    ce::v2::static_error{.code = ce::v2::errc::invalid_utf8}));
    expect(true);
  };
};
...
const boost::ut::suite<"static-error-widens-into-error"> static_error_widens = [] {
  ...
  "widening keeps every part"_test = [] {
    constexpr ce::v2::static_error diagnosis{
        .code = ce::v2::errc::invalid_attribute_name,
        .detail = "extension names must match [a-z0-9]+",
        .where = "extension",
    };
    const ce::v2::error owned = ce::v2::widen(diagnosis);
    expect(owned.code == ce::v2::errc::invalid_attribute_name);
  ...
  "error is still an aggregate"_test = [] {
    ...
    const ce::v2::error built{.code = ce::v2::errc::parse_error, .detail = "d", .where = "w"};
    expect(built.code == ce::v2::errc::parse_error);
  };

  "a diagnosis converts into a failed result of any type"_test = [] {
    constexpr ce::v2::static_error diagnosis{
        .code = ce::v2::errc::out_of_range,
        .detail = "outside the representable range",
    };

    const ce::v2::result<int> number = ce::v2::fail(diagnosis);
    expect(!number.has_value());
    if (!number) {
      expect(number.error().code == ce::v2::errc::out_of_range);
      expect(number.error().detail == "outside the representable range");
    }

    const ce::v2::result<void> nothing = ce::v2::fail(diagnosis);
  ...
  "the offending value can be attached on the runtime path"_test = [] {
    constexpr ce::v2::static_error diagnosis{
        .code = ce::v2::errc::invalid_timestamp,
        .detail = "not an RFC 3339 date-time",
        .where = "time",
    };
    const ce::v2::result<int> parsed = ce::v2::fail(diagnosis, std::string{"not-a-date"});
